...
読み込み完了後は、命令が一つもない長い空白を自動で早送りし、次の命令が現れる直前から通常再生します。**Go to cycle → Skip empty gaps** で切り替えられ、初期状態はONです。早送りしたサイクル数を一時表示します。長い命令待ち、完了・フラッシュの演出、読み込み途中、**Loop window**、手動の1サイクル送りは早送りしません。
