...
Blocks の命令は、待機・移動・実行を通して同じ大きさを保ちます。ユニット上では接地し、ステージ間は両端の高さを滑らかにつないで移動します。移動に合わせて回転し、カット面の向きと明暗が変わります。待機中・一時停止中は回転も止まり、**Motion effects** を OFF にすると向きを固定できます。
