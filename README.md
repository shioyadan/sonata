...
読み込みはバックグラウンドで進み、解析できた命令から表示・再生・区間移動できます。途中表示には後から命令が追加されるため、画面に読込み中と表示します。完了時も再生位置を保って最新の内容に更新します。**Cancel** で読み込みを取り消し、デモへ戻れます。数GBのログでも全文の文字列や描画用の全命令配列を作らず、Konataの圧縮ページ保存を使います。

画面下部は二段の時間軸です。上段の **PARSED TRACE** には読込み済みの全体活動と現在の表示区間、下段にはその区間の詳細と再生位置を表示します。上段のクリック・ドラッグで遠くへ移動し、下段のホイールまたは **− / +** で時間軸を拡大縮小できます。下段の **Shift＋ドラッグ／ホイール** は区間の平行移動です。初期幅は128サイクル、拡大縮小は16〜512サイクルです。

再生と1サイクル送りは区間の境界を越えて続きます。次の区間を先読みし、読込みに追いついた場合は続きを待ち、ファイル末尾で停止します。**Navigate** にはサイクル番号での直接移動、スレッド、履歴の **Back / Forward**、**Bookmark**、明示的な **Loop window** をまとめています。同梱デモは従来どおり末尾でループします。

右側の **Find in trace** からPC・命令文字列・命令ID、取消命令、一定以上の寿命を持つ命令を検索できます。選択スレッドの結果をID順に40件ずつ表示し、選ぶとその命令の周辺へ移動します。寿命はfetchから最後の記録までの長さで、キャッシュミスや待機原因を表すものではありません。ブックマークには名前を付けられ、同じ名前・サイズ・更新時刻のファイルを再び開くと復元します。ファイル自体は保存しません。

ステージの役割はgem5・RSDの記録形式、その他のログでは命令の進行順から判定します。容量は観測した区間からの推定です。同じ前段構成・スレッドでは観察中に得た最大値を保ち、区間移動のたびに縮まないようにしています。役割を判定できないステージは記録順の前段として表示し、未観測のレジスタ値やTop-down分類を補いません。未完了の命令はコミットさせません。同時に表示できる密度やFIFO順序には制約があり、対応できない区間は理由を表示します。大きなファイルも圧縮後の記録はメモリに保持するので、扱えるサイズは内容と端末の空きメモリに依存します。[読込みの構造と制約](docs/trace-import.md)を参照してください。
...
検証画像は `artifacts/screenshots/` に出力します。描画検査は HTML だけを一時フォルダへコピーし、外部アクセスを禁止して実行します。通常のCIはモデル・ビルド・型検査と、`npm run test:smoke` による基本的な描画・操作の確認を行います。全描画検査はローカル、またはGitHub Actionsの手動実行で `full_render` を選んで行います。詳細は [開発ガイド](docs/development.md) にまとめています。
