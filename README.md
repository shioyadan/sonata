...
命令がfetch段階などで長く静止している場合は、**Speed up waits**（初期ON）で選択速度の16倍に加速し、次の記録上の変化の手前で通常速度に戻します。加速中は **Fast-forwarding wait · 16×** と表示します。待機の長さを通常速度で観察する場合はOFFにしてください。パイプ内の移動、完了・フラッシュの演出、未完了命令、読み込み途中、**Loop window**、手動の1サイクル送りは加速しません。
