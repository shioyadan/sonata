...
読み込み完了後は、命令が一つもない長い空白を自動で飛ばし、次の命令が現れる直前から通常再生します。**Go to cycle → Skip empty gaps** で切り替えられ、初期状態はONです。飛ばしたサイクル数を一時表示します。

命令がfetch段階・LOAD WAIT・ROBなどで待機し、状態が変わらない場合は、**Speed up waits**（初期ON）で選択速度の16倍に加速し、次の記録上の変化の手前で通常速度に戻します。コミット後のストア書込み待ちも対象です。加速中は **Fast-forwarding wait · 16×** と表示します。待機の長さを通常速度で観察する場合はOFFにしてください。パイプ内の移動、完了・フラッシュの演出、未完了命令、読み込み途中、**Loop window**、手動の1サイクル送りは加速しません。
