...
サンプルを試す場合は `npm start` で `http://127.0.0.1:4173` を開き、一覧から選んでください。5本のデモは `dist/samples/` に生成され、選択時に必要なJSONだけを取得します。`#demo=rename-rush` のようなリンクで直接開くこともできます。ソース変更後はサーバーを再起動してください。
...
ステージの構成は読み込んだ範囲からスレッドごとに蓄積します。役割はgem5・RSD・Onikiriの既知の記録規則、その他のログでは有界な命令標本の進行順から判定します。容量は標本と表示区間で観測した同時数からの推定です。観察中の最大値と、重なる区間の命令位置を引き継ぎ、区間移動による配置の変化を抑えます。役割を判定できないステージは記録順の前段として表示し、未観測のレジスタ値やTop-down分類を補いません。未完了の命令はコミットさせません。同時に表示できる密度やFIFO順序には制約があり、対応できない区間は理由を表示します。大きなファイルも圧縮後の記録はメモリに保持するので、扱えるサイズは内容と端末の空きメモリに依存します。[読込みの構造と制約](docs/trace-import.md)を参照してください。
...
dist/samples/            選択時に取得するデモJSON（Git 対象外）
artifacts/              検証画像・レポート（Git 対象外）
inputs/                 再抽出用の元ログ（Git 対象外）
work/                   ローカル作業メモ・引き継ぎ資料（Git 対象外）
```

ソースの分割と編集先は [構造の説明](docs/architecture.md) を参照してください。アプリ本体は単一 HTML に結合し、デモは別のJSONとして配布します。通常のビルド・検証には Konata のチェックアウトや元ログは不要です。デモを再抽出する場合だけ、[再生成の手順](data/README.md#再生成) に沿って元ログを用意してください。
