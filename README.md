...
ステージの構成は読み込んだ範囲からスレッドごとに蓄積します。役割はgem5・RSDの記録形式、その他のログでは有界な命令標本の進行順から判定します。容量は標本と表示区間で観測した同時数からの推定です。観察中の最大値と、重なる区間の命令位置を引き継ぎ、区間移動による配置の変化を抑えます。役割を判定できないステージは記録順の前段として表示し、未観測のレジスタ値やTop-down分類を補いません。未完了の命令はコミットさせません。同時に表示できる密度やFIFO順序には制約があり、対応できない区間は理由を表示します。大きなファイルも圧縮後の記録はメモリに保持するので、扱えるサイズは内容と端末の空きメモリに依存します。[読込みの構造と制約](docs/trace-import.md)を参照してください。
