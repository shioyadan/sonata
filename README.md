...
詳細側の **Previous / Next** は区間移動、ホイールまたは **− / +** は時間軸の拡大縮小、**Shift＋ドラッグ／ホイール** は区間の平行移動です。初期幅は128サイクル、拡大縮小は16〜512サイクルです。詳細バーでの時刻指定と1サイクル送りは、観察のため再生を止めます。

再生と1サイクル送りは区間の境界を越えて続きます。次の区間を先読みし、読込みに追いついた場合は続きを待ち、ファイル末尾で停止します。全体側の **Start / Latest** は先頭／読込済み末尾の区間へ移動します。**Go to cycle** にはサイクル番号、表示幅、スレッド、履歴の **Back / Forward**、**Bookmark**、明示的な **Loop window** をまとめています。同梱デモは従来どおり末尾でループします。
