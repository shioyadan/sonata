...
- 通常のビルドは Node の標準機能だけで完結させる。`dist/sonata.html` はスタイル・コード・解析器・ライセンスを含み、単体でローカル File をオフライン読込みできる。生トレースは `dist/samples/*.log.gz` に分離し、HTTP(S) で選んだサンプルだけを取得する。5デモで4ファイルを共有し、Fileと同じWorker・Konata core・区間変換で解析する。
