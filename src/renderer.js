...
function createRenderer({activity,camera,clock,gpu,session}) {
    const {programs}=gpu;
    const shadowPass={surfaceTexture:null,materialShadow:null,lightProjection:null,pieceShadow:null,pieceShadowKey:"",pieceShadowUpdates:0};
    const {gl}=gpu;
    ...
    function buildMaterialShadow(){
        ...
        shadowPass.lightProjection=multiply(projection,lookAt(session.style.surface.light.map(v=>v*26),[0,0,0]));
...
// 固定面・木の材質と影を受ける面。
function createSurfaceShaders(program) {
...
function createCrystalShaders(program,surfaceLighting,crystalTransmission) {
...
// 光点・文字・ブルームと最終合成。
function createEffectShaders(program) {
...
module.exports={createGpu,createRenderer};
