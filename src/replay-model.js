...
const {stageTransition}=require("./geometry.js");
...
function createReplay({samples}) {
    ...
    function loadTrace(key) {
        replay.trace=samples.find((s)=>s.key===key)||samples[0];
        replay.ops=replay.trace.ops.map((t,index)=>{
            ...
            return {id,rid,index,fetch,end,flush:!!flush,label,stages,allocation,issue,completion,execution,kind:instructionKind(label),
                reads:replay.trace.evidence?.registers?.origin==="gem5"?[...new Set((replay.trace.evidence.registers.reads??[]).filter(r=>r.id===id).map(r=>r.cycle))].map(time=>({start:time,end:time+.7,sources:replay.trace.evidence.registers.reads.filter(r=>r.id===id&&r.cycle===time).map(r=>({physical:r.physical,hex:r.hex}))})):source.filter(s=>s[0]==="Rr").map(s=>({start:s[2],end:s[3]})),
                sourceRegisters:replay.trace.evidence?.scheduling.ops.find(o=>o.id===id)?.sources??[]};
        });
        allocateRenameSlots();
        ...
        replay.feedReplay=createFeedReplay(replay.feedOps,{firstCycle:replay.trace.firstCycle,lastCycle:replay.trace.lastCycle,rows:feedRows,lead:feedLead});
        allocateSlots(o=>o.allocation,o=>o.issue??o.end,"issueSlot");
        replay.dependencyReplay=createDependencyReplay(replay.ops,replay.trace.evidence?.scheduling,replay.trace.structure.queueCapacity);
        replay.registerReplay=createRegisterReplay(replay.trace.evidence?.registers);
        const regs=replay.trace.evidence?.registers;
        ...
        replay.robReplay=createRobReplay(replay.ops,replay.trace.structure.robCapacity);
        ...
        replay.flushEvents=[...new Set(replay.ops.filter(o=>o.flush&&o.end>=replay.trace.firstCycle&&o.end<=replay.trace.lastCycle).map(o=>o.end))].sort((a,b)=>a-b);
        replay.branchRecoveries=findRecoveryBranches(replay.ops,replay.trace.demo.events,replay.flushEvents,replay.trace.parser.startsWith("gem5"));

        replay.activity=Array.from({length:replay.trace.lastCycle-replay.trace.firstCycle+1},(_,i)=>{
            const t=replay.trace.firstCycle+i;
...
module.exports={createRobReplay, memoryCompletions, createFeedReplay, codeRewindDuration, sampleTopDown,flushPlaybackRate,advancePlayback,measureTransfers,createDependencyReplay,createRegisterReplay,findRecoveryBranches,createReplay,feedRows,feedLead};
globalThis.sonataReplay=module.exports;
