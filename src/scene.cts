...
interface StyleBase {label:string;palette:Record<"integer"|"memory"|"branch"|"red"|"blue"|"floor",Vector>;background:Vector;bounds:Record<BoundKey,string>;timeline:string[]}
type SceneStyle=StyleBase & ({matte:false}|{matte:true;structure:Record<"body"|"base"|"rail"|"recess"|"wire"|"ink",Vector>;surface:{wood:Vector;roughness:number;grain:number;light:Vector;pieceShadow:number}});
interface SceneNode {id:string;label:string;x:number;z:number;w:number;d:number;h:number;color:Vector;detail:string;names?:string[];pipeCount?:number;compact?:boolean;matrixDepth?:number;mapWords?:number;instructionSlots?:number;grid?:{rows:[Vector,Vector][];columns:[Vector,Vector][]};element?:HTMLDivElement}
interface Connection {from:string;to:string;color:Color;peak:number;lanes:{source:Vector;target:Vector}[]}
interface TriangleData {triangles:number[];materials:number[];shadows:number[]}
...
interface SceneOptions {gpu?:renderer.Gpu;replay:sonataReplay.Replay;session:{style:SceneStyle}}
...
function createScene({gpu,replay,session}:SceneOptions) {
    const scene:{nodes:Map<string,SceneNode>;connections:Connection[];transferProfile:ReturnType<typeof sonataReplay.measureTransfers>}={nodes:new Map(),connections:[],transferProfile:new Map()};
    function executionLane(node:Pick<SceneNode,"x"|"z"|"h"|"w"|"d"|"pipeCount">,index:number):{inlet:Vector;outlet:Vector;radius:number} {
    ...
    function makeNode(id:string,label:string,x:number,z:number,w:number,d:number,h:number,color:Vector,detail:string):SceneNode {
    ...
    function crossesDependencyGrid(a:Vector,b=a) {
        const n=scene.nodes.get("issue")!,low:Vector=[n.x-n.w*.30,n.h+.06,n.z-n.matrixDepth!/2-.06],high:Vector=[n.x+n.w*.42,n.h+1.2,n.z+n.matrixDepth!/2+.06];
    ...
    function renameWordLayout(index:number):{bits:number;bank:number;start:Vector;end:Vector;halfWidth:number}{
    ...
    function wakeBusEntry():Vector{const n=scene.nodes.get("issue")!;return [n.x-n.w*.50,n.h+.3,n.z-n.d*.5-.22];}

    function wakeColumnHead(column:number):Vector{const n=scene.nodes.get("issue")!,p=matrixPosition(0,column);return [p[0],n.h+.3,n.z-n.d*.5-.22];}

    function issueRowExit(slot:number):Vector{const n=scene.nodes.get("issue")!,p=matrixPosition(slot);return [n.x+n.w*.5+.06,p[1],p[2]];}
    ...
    function issuePath(issue:Issue){
        ...
        const signal:Vector[]=issue.column===null?[]:[exit,[exit[0],exit[1],n.z+n.d*.5+.18],
    ...
    function wakePath(source:Vector,progress:number,producerID?:number):Vector {
        ...
        return source.map((v,i)=>q*q*q*v+3*q*q*t*a[i]+3*q*t*t*b[i]+t*t*t*target[i]) as Vector;
    }
    ...
    // 固定部品の形状・材質・ラベルと接地面。
    function vertex(out:VertexData,p:Color,color:Color,alpha=1) { (out.triangles??out).push(...p, ...color, alpha); }
    function line(out:VertexData,a:Color,b:Color,color:Color,alpha=1) { vertex(out, a, color, alpha); vertex(out, b, color, alpha); }
    function point(out:number[],p:Color,color:Color,size:number,alpha=1) { out.push(...p, ...color, alpha, size); }
    const baseLevels={table:-.87,lowerBoard:-.71,board:-.29,plinth:-.09};
    // 通常の三角形と材質付きの部品を同じ組立処理から分ける。
    function beveledBlock(out:VertexData,x:number,y:number,z:number,w:number,h:number,d:number,color:Color,alpha=1,material=0,supportY=y) {
    ...
    function flatQuad(tris:VertexData,x:number,y:number,z:number,w:number,d:number,color:Color,alpha=1) {
        const p=[[x-w/2,y,z-d/2],[x+w/2,y,z-d/2],[x+w/2,y,z+d/2],[x-w/2,y,z+d/2]];
        for(const k of [0,1,2,0,2,3])vertex(tris,p[k],color,alpha);
    }
    function contactShadow(tris:VertexData,x:number,y:number,z:number,w:number,d:number,spread:number,opacity=.22) {
    ...
    function ring(out:VertexData,x:number,y:number,z:number,radius:number,color:Color,alpha=1,start=0,end=TAU,segments=80) {
    ...
    function box(tris:TriangleData,lines:number[],x:number,y:number,z:number,w:number,h:number,d:number,color:Color,glow=.55,supportY=y) {
    ...
    function housing(tris:TriangleData,lines:number[],x:number,y:number,z:number,w:number,h:number,d:number,color:Color,glow=.4,supportY=y) {
        ...
        const face=(p:Color[],c:Color)=>{for(const k of [0,1,2,0,2,3])vertex(tris,p[k],c);};
        for(let i=0;i<8;i++){
    ...
    function executionModule(tris:TriangleData,lines:number[],node:SceneNode) {
        ...
        for(let k=0;k<node.pipeCount!;k++){
            ...
            const intake:Vector=[nodePort(node,false)[0],a[1],a[2]],outlet:Vector=[nodePort(node,true)[0],b[1],b[2]];
    ...
    function matrixModule(tris:TriangleData,lines:number[],n:SceneNode) {
        ...
        for(let r=0;r<replay.trace!.structure.queueCapacity;r++){
            const p=matrixPosition(r),segment:[Vector,Vector]=[[p[0],y,p[2]],[x1,y,p[2]]];
        ...
        for(let c=0;c<replay.dependencyReplay!.columnCount;c++){
            const x=matrixPosition(0,c)[0],segment:[Vector,Vector]=[[x,y,z0],[x,y,z1]];
    ...
    function buildWorld() {
        buildLayout();
        const lines:number[]=[],stars:number[]=[],shadows:number[]=[],tris:TriangleData={triangles:[],materials:[],shadows};
        if(session.style.matte){
        ...
        if(scene.nodes.has("memory-wait")){
            const n=scene.nodes.get("memory-wait")!,source:Vector=[n.x,n.h+.34,n.z];
        ...
        if(session.style.matte){
            ...
            for(let i=42;i<tris.triangles.length;i+=21)surfaces.push([0,7,14].map(k=>tris.triangles.slice(i+k,i+k+3) as Vector) as geometry.GroundTriangle);
            for(let i=0;i<tris.materials.length;i+=12){
                ...
                for(let j=0;j<gpu!.roundedGeometry.length;j+=18){
                    ...
                    surfaces.push([0,6,12].map(k=>[0,1,2].map(a=>m[a]+gpu!.roundedGeometry[j+k+a]*(m[3+a]-m[10])+gpu!.roundedGeometry[j+k+3+a]*m[10]) as Vector) as geometry.GroundTriangle);
                }
            }
        ...
        for (const n of scene.nodes.values()) {
            ...
            label.dataset.label=({"register-read":"REG FILE","rob":"ROB","memory-wait":"MEM WAIT"} as Record<string,string>)[n.id]??n.label;
...
declare global {var sonataStyles:typeof styles}
export = sceneModule;
