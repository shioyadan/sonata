...
const {clamp,mix,smooth,rgb,multiply,perspective,lookAt,instructionRadius,createPaths}=geometry;
...
interface Session {
    cycle:number;playing:boolean;speed:number;selectedID:number|null;visualStyle:sceneModel.StyleKey;style:sceneModel.Style;
    reducedMotion:boolean;bloom:number;trails:boolean;instructionStream:boolean;
}
type ElementFor<ID extends string>=ID extends "scene"|"activity"?HTMLCanvasElement
    :ID extends "license-panel"|"mobile-panel"?HTMLDialogElement
    :ID extends "trace-select"|"speed"?HTMLSelectElement
    :ID extends "timeline"|"bloom"?HTMLInputElement
    :ID extends "show-highlight"|"next-flush"?HTMLButtonElement:HTMLElement;
...
function start(gl:WebGL2RenderingContext){
    ...
    const session:Session={cycle:0,playing:true,speed:4,selectedID:null,visualStyle:"neon",style:styles.neon,
        reducedMotion:false,bloom:1.7,trails:true,instructionStream:true};
    const clock={setPlaying,setCycle,lastPlayback:{seconds:0,cycles:0,rate:1},lastTime:performance.now(),artTime:0,frameCount:0,fpsTime:performance.now(),fps:0,nextUI:0,animationID:null as number|null};
    ...
    function step(delta:number){setPlaying(false);setCycle(Math.floor(session.cycle)+delta);}
    ...
    function setVisualStyle(key:sceneModel.StyleKey){
        ...
        document.querySelector<HTMLMetaElement>('meta[name="color-scheme"]')!.content=session.style.matte?"light":"dark";
        for(const el of document.querySelectorAll<HTMLElement>("[data-bound]")){
            if(el.closest(".bound-bar,.bound-scene-key"))el.style.setProperty("--bound-color",session.style.bounds[el.dataset.bound as keyof typeof session.style.bounds]);
        }
        for(const button of document.querySelectorAll<HTMLButtonElement>("[data-style-choice]"))button.setAttribute("aria-pressed",String(button.dataset.styleChoice===key));
    ...
    document.querySelectorAll<HTMLButtonElement>("[data-style-choice]").forEach(button=>button.addEventListener("click",()=>setVisualStyle(button.dataset.styleChoice as sceneModel.StyleKey)));
    ...
    document.querySelectorAll<HTMLButtonElement>("[data-view]").forEach(b=>b.addEventListener("click",()=>camera.setCamera(b.dataset.view as CameraMode)));
    ...
    function updateLabels(dt=0) {
        ...
        if(camera.compactMedia.matches){
            ...
            const priority=(n:sceneModel.Node)=>({issue:0,"register-read":1,rob:2} as Record<string,number>)[n.id]??3;
    ...
    function updateUI() {
        ...
        $("active-count").textContent=String(activity.currentStats.active!.length);$("ipc-value").textContent=activity.currentStats.ipc!.toFixed(2);
        ...
        for(const [name,count,capacity] of [["issue",activity.currentStats.issued!.length,replay.trace!.structure.queueCapacity],["rob",activity.currentStats.rob!.length,replay.trace!.structure.robCapacity]] as const){
        ...
        if(op){
            const stage=paths.stageAt(op,session.cycle),stageIndex=op.stages.indexOf(stage as sonataReplay.Stage);
    ...
    function showTrace() {
        $("timeline").min=String(replay.trace!.firstCycle);$("timeline").max=String(replay.trace!.lastCycle);
    ...
    function createDiagnostics(){
        return {
            ...
            captureAt(t:number){clock.setPlaying(false);clock.setCycle(t);return {cycle:session.cycle,active:activity.currentStats.active!.length};}
        };
    }
...
// カメラの補間・投影とマウス／タッチ操作。
function createCamera({gpu,onPick}:{gpu:rendering.Gpu;onPick(x:number,y:number):void}) {
    const camera={
        cameraMode:"orbit" as CameraMode,azimuth:.20,elevation:.73,radius:32.5,targetAzimuth:.20,targetElevation:.73,targetRadius:32.5,
        focus:[0,0,0] as Vec3,targetFocus:[0,0,0] as Vec3,autoOrbit:true,
        compactMedia:matchMedia("(max-width:760px), (max-width:1000px) and (max-height:600px)"),
        viewProjection:new Float32Array(16),eye:[0,20,30] as Vec3
    };
    ...
    function setCamera(mode:CameraMode){
        ...
        document.querySelectorAll<HTMLButtonElement>("[data-view]").forEach(b=>b.setAttribute("aria-pressed",String(b.dataset.view===mode)));
        gpu.resize();
    }
    ...
    const pointers=new Map<number,{x:number;y:number;startX:number;startY:number;moved:boolean;zoom:boolean}>();
    ...
    function moveFocus(dx:number,dy:number,units:number){
        ...
        camera.targetFocus=camera.targetFocus.map((v,i)=>clamp(v+(right[i]*dx-up[i]*dy)*units,-[20,12,14][i],[20,12,14][i])) as Vec3;
    }
    ...
    for(const type of ["pointerup","pointercancel","lostpointercapture"] as const)gpu.canvas.addEventListener(type,endPointer);
    ...
    function update(dt:number,artTime:number) {
        ...
        camera.eye=[Math.sin(a)*Math.cos(camera.elevation)*distance,Math.sin(camera.elevation)*distance,Math.cos(a)*Math.cos(camera.elevation)*distance].map((value,i)=>value+camera.focus[i]) as Vec3;
