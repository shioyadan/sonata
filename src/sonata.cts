...
function start(gl: WebGL2RenderingContext) {
    ...
    function createDiagnostics() {
        return {
            ...
            get schedulers() {
                return replay.schedulers.map((bank) => {
                    const node = scene.nodes.get(bank.id)!;
                    return {
                        ...bank,
                        label: node.label,
                        bounds: { x: node.x, z: node.z, w: node.w, d: node.d, h: node.h },
                        grid: node.grid && {
                            rows: node.grid.rows.map((segment) => segment.map((p) => [...p])),
                            columns: node.grid.columns.map((segment) => segment.map((p) => [...p]))
                        }
                    };
                });
            },
