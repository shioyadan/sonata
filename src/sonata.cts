...
function createCamera({ gpu, onPick }: { gpu: rendering.Gpu; onPick(x: number, y: number): void }) {
    ...
    function project(p: Vec3): Vec3 {
        ...
        return [
            (((m[0] * p[0] + m[4] * p[1] + m[8] * p[2] + m[12]) / w) * gpu.cssWidth) / 2 + gpu.cssWidth / 2,
            ((-(m[1] * p[0] + m[5] * p[1] + m[9] * p[2] + m[13]) / w) * gpu.cssHeight) / 2 + gpu.cssHeight / 2,
            w
        ];
    }
    ...
    function toggleAuto(value = !camera.autoOrbit) {
        ...
        $("auto-camera").setAttribute("aria-pressed", String(camera.autoOrbit));
        $("auto-camera").lastElementChild!.textContent = camera.autoOrbit ? "ON" : "OFF";
    }
    ...
    gpu.canvas.addEventListener("pointerdown", (event) => {
        ...
        gpu.canvas.setPointerCapture(event.pointerId);
        if (pointers.size === 2) {
    ...
    gpu.canvas.addEventListener("pointermove", (event) => {
        ...
        if (pointers.size === 2) {
            const next = touchPair(),
                ...
                rect = gpu.canvas.getBoundingClientRect();
            ...
            const scale = camera.targetRadius / oldRadius,
                units =
                    (2 * Math.tan(0.66 / 2) * oldRadius * Math.max(1, 1.48 / (gpu.cssWidth / gpu.cssHeight))) /
                    gpu.cssHeight;
            moveFocus(
    ...
    for (const type of ["pointerup", "pointercancel", "lostpointercapture"] as const)
        gpu.canvas.addEventListener(type, endPointer);
    ...
    function update(dt: number, artTime: number) {
        ...
        camera.viewProjection = multiply(
            perspective(gpu.cssWidth / gpu.cssHeight, distance),
