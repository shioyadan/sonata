...
interface VertexBuffer {
    vao: WebGLVertexArrayObject | null; vbo: WebGLBuffer | null; count: number; stride: number;
    instances?: false; vertices?: never;
}
interface InstanceBuffer extends Omit<VertexBuffer,"instances" | "vertices"> { instances: true; vertices: number }
type DrawBuffer = VertexBuffer | InstanceBuffer;
interface GpuProgram { p: WebGLProgram; u(name: string): WebGLUniformLocation | null }
type ProgramFactory = (vertex: string, fragment: string) => GpuProgram;
interface DisposableTarget {
    fbo: WebGLFramebuffer | null; texture?: WebGLTexture | null; coverage?: WebGLTexture | null;
    depthBuffer?: WebGLRenderbuffer | null; colorBuffer?: WebGLRenderbuffer | null; w: number; h: number;
}
interface RenderTarget extends DisposableTarget { texture: WebGLTexture | null }
interface MultisampleTarget extends DisposableTarget { depthBuffer: WebGLRenderbuffer | null; colorBuffer: WebGLRenderbuffer | null }
interface GpuState {
    gl: WebGL2RenderingContext; canvas: HTMLCanvasElement;
    renderWidth: number; renderHeight: number; cssWidth: number; cssHeight: number; pixelRatio: number; contextLost: boolean;
    sceneTarget: RenderTarget | null; sceneMultisample: MultisampleTarget | null; bloomA: RenderTarget | null; bloomB: RenderTarget | null;
    staticTriangles: VertexBuffer | null; staticLines: VertexBuffer | null; staticStars: VertexBuffer | null; staticShadows: VertexBuffer | null;
    staticMaterials: InstanceBuffer | null; msaaSamples: number; maxRenderSide: number;
}
interface SurfaceStyle { light: Vec3; roughness: number; grain: number; pieceShadow: number }
type RenderStyle = { background: Vec3 } & ({ matte: false; surface?: undefined } | { matte: true; surface: SurfaceStyle });
interface RenderSession { cycle: number; reducedMotion: boolean; bloom: number; style: RenderStyle }
interface RenderCamera { eye: Vec3; viewProjection: Float32Array<ArrayBuffer>; update(dt: number, artTime: number): void }
interface ShadowPass {
    surfaceTexture: WebGLTexture | null; materialShadow: RenderTarget | null; lightProjection: Float32Array<ArrayBuffer> | null;
    pieceShadow: RenderTarget | null; pieceShadowKey: string; pieceShadowUpdates: number;
}
const crystalTransmission=.40;

// 起動側で有効な WebGL 2 を取得してから資源を作る。取得失敗・context loss は起動側が扱う。
function createGpu({gl,canvas,onResize}: {gl: WebGL2RenderingContext; canvas: HTMLCanvasElement; onResize(): void}) {
    function program(vertex: string, fragment: string): GpuProgram {
        const shaders = ([[gl.VERTEX_SHADER, vertex], [gl.FRAGMENT_SHADER, fragment]] as const).map(([type, source]) => {
        ...
        return { p, u(name: string) { if (!uniforms.has(name)) uniforms.set(name, gl.getUniformLocation(p, name)); return uniforms.get(name)!; } };
    }
    ...
    const gpu: GpuState={
        gl,canvas,renderWidth:0,renderHeight:0,cssWidth:0,cssHeight:0,pixelRatio:1,contextLost:false,
        sceneTarget:null,sceneMultisample:null,bloomA:null,bloomB:null,
        staticTriangles:null,staticLines:null,staticStars:null,staticShadows:null,staticMaterials:null,
    ...
    function deleteBuffer(b: DrawBuffer | null) { if (b) { gl.deleteBuffer(b.vbo); gl.deleteVertexArray(b.vao); } }
    ...
    for(let axis=0;axis<3;axis++)for(const sign of [-1,1]){
        ...
        for(let i=0;i<3;i++)for(let j=0;j<3;j++){
            const corners=[[i,j],[i+1,j],[i+1,j+1],[i,j+1]].map(([a,b])=>{
                const p: Vec3=[0,0,0];p[axis]=sign;p[u]=grid[a];p[v]=grid[b];
                return [...p.map(Math.sign),...normalize(p.map(c=>Math.abs(c)>.75?Math.sign(c):0) as Vec3)];
            });
    ...
    function resize() {
        gpu.cssWidth = Math.max(1, canvas.clientWidth); gpu.cssHeight = Math.max(1, canvas.clientHeight);
        gpu.pixelRatio = Math.min(devicePixelRatio || 1, 2.5, Math.sqrt(8388608/(gpu.cssWidth*gpu.cssHeight)), gpu.maxRenderSide/gpu.cssWidth, gpu.maxRenderSide/gpu.cssHeight);
        const w = Math.round(gpu.cssWidth * gpu.pixelRatio), h = Math.round(gpu.cssHeight * gpu.pixelRatio);
        if (w === gpu.renderWidth && h === gpu.renderHeight) return;
        gpu.renderWidth = canvas.width = w; gpu.renderHeight = canvas.height = h;
    ...
    return Object.assign(gpu,{programs,buffer,upload,deleteBuffer,destroyTarget,resize,materialBuffer,roundedGeometry,movingLines,particles,analysisSurface,registerSurface,instructionPieces,glyphTexture,feedType,unravelType});
}

// 固定バッファは buildWorld、描画先は resize、Blocks の影は buildMaterialShadow 後に利用する。
// 以下の非 null 参照はこの初期化順に対応し、未初期化の状態は公開する GPU の型にも残す。
function createRenderer({activity,camera,clock,gpu,session}: {
    activity: {drawDynamic(dt: number): number}; camera: RenderCamera; clock: {artTime: number};
    gpu: ReturnType<typeof createGpu>; session: RenderSession;
}) {
    const {programs}=gpu;
    const shadowPass: ShadowPass={surfaceTexture:null,materialShadow:null,lightProjection:null,pieceShadow:null,pieceShadowKey:"",pieceShadowUpdates:0};
    const {gl}=gpu;
    function createSurfaceTexture(){
        ...
        const grid=(columns: number,rows: number,seed: number)=>{
            ...
            return (x: number,y: number)=>{
                ...
                const sample=(a: number,b: number)=>values[((b%rows+rows)%rows)*columns+(a%columns+columns)%columns];
    ...
    function shadowTarget(withCoverage=false): RenderTarget{
        const size=Math.min(2048,gpu.maxRenderSide),texture=gl.createTexture(),fbo=gl.createFramebuffer();
    ...
    function buildMaterialShadow(){
        ...
        if(!session.style.matte){gpu.destroyTarget(shadowPass.materialShadow);gpu.destroyTarget(shadowPass.pieceShadow);shadowPass.materialShadow=shadowPass.pieceShadow=null;return;}
        ...
        shadowPass.lightProjection=multiply(projection,lookAt(session.style.surface.light.map(v=>v*26) as Vec3,[0,0,0]));
        gl.bindFramebuffer(gl.FRAMEBUFFER,shadowPass.materialShadow.fbo);gl.viewport(0,0,shadowPass.materialShadow.w,shadowPass.materialShadow.h);
        ...
        gl.useProgram(programs.shadowProgram.p);gl.uniformMatrix4fv(programs.shadowProgram.u("uMatrix"),false,shadowPass.lightProjection);
        gl.bindVertexArray(gpu.staticMaterials!.vao);gl.drawArraysInstanced(gl.TRIANGLES,0,gpu.roundedGeometry.length/6,gpu.staticMaterials!.count);
    ...
    function updatePieceShadow(){
        ...
        gl.bindFramebuffer(gl.FRAMEBUFFER,shadowPass.pieceShadow.fbo);gl.viewport(0,0,shadowPass.pieceShadow.w,shadowPass.pieceShadow.h);
        ...
        gl.uniform3fv(programs.pieceShadowProgram.u("uEye"),session.style.surface!.light.map(v=>v*26));
        ...
        shadowPass.pieceShadowKey=key;shadowPass.pieceShadowUpdates++;
    }

    function drawBuffer(b: DrawBuffer,mode: number,p: GpuProgram) {
        if(!b.count)return;
        gl.useProgram(p.p);gl.uniformMatrix4fv(p.u("uMatrix"),false,camera.viewProjection);
        if(p===programs.materialProgram||p===programs.pieceReceiverProgram){
            ...
            gl.activeTexture(gl.TEXTURE4);gl.bindTexture(gl.TEXTURE_2D,shadowPass.pieceShadow!.texture);gl.uniform1i(p.u("uPieceDepth"),4);
            gl.activeTexture(gl.TEXTURE5);gl.bindTexture(gl.TEXTURE_2D,shadowPass.pieceShadow!.coverage!);gl.uniform1i(p.u("uPieceCoverage"),5);
            gl.uniform1f(p.u("uPieceStrength"),session.style.surface!.pieceShadow*(1-crystalTransmission*.5));
        }
        if(p===programs.materialProgram||p===programs.pieceProgram){
            gl.uniform3fv(p.u("uEye"),camera.eye);gl.uniform3fv(p.u("uLight"),session.style.surface!.light);
            gl.uniform1f(p.u("uRoughness"),session.style.surface!.roughness);gl.uniform1f(p.u("uGrain"),session.style.surface!.grain);
            gl.uniformMatrix4fv(p.u("uLightMatrix"),false,shadowPass.lightProjection!);
            gl.activeTexture(gl.TEXTURE2);gl.bindTexture(gl.TEXTURE_2D,shadowPass.materialShadow!.texture);
            gl.uniform1i(p.u("uShadow"),2);gl.uniform1f(p.u("uShadowTexel"),1/shadowPass.materialShadow!.w);
            gl.activeTexture(gl.TEXTURE3);gl.bindTexture(gl.TEXTURE_2D,shadowPass.surfaceTexture);gl.uniform1i(p.u("uSurfaceTexture"),3);
        }
        if(p===programs.pieceProgram){
            gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,gpu.bloomA!.texture);gl.uniform1i(p.u("uBackground"),0);
            gl.uniform1f(p.u("uScale"),gpu.renderHeight/(2*Math.tan(.33)));
        }
        if(p===programs.pointProgram){gl.uniform1f(p.u("uScale"),gpu.renderHeight*.042);gl.uniform1i(p.u("uMatte"),session.style.matte?1:0);}
    ...
    function blur(source: RenderTarget,destination: RenderTarget,dx: number,dy: number,extract: number) {
        ...
        gl.useProgram(programs.blurProgram.p);gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,source.texture);
        gl.uniform1i(programs.blurProgram.u("uTexture"),0);gl.uniform2f(programs.blurProgram.u("uDirection"),dx,dy);gl.uniform1f(programs.blurProgram.u("uExtract"),extract);
    ...
    function render(dt=0) {
        ...
        gl.bindFramebuffer(gl.FRAMEBUFFER,(gpu.sceneMultisample??gpu.sceneTarget!).fbo);gl.viewport(0,0,gpu.renderWidth,gpu.renderHeight);
        gl.depthMask(true);gl.clearColor(...session.style.background,1);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);
        ...
        gl.depthMask(false);gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,session.style.matte?gl.ONE_MINUS_SRC_ALPHA:gl.ONE);
        ...
        if(session.style.matte&&gpu.instructionPieces.count){
            ...
            if(gpu.sceneMultisample){
                gl.bindFramebuffer(gl.READ_FRAMEBUFFER,gpu.sceneMultisample.fbo);gl.bindFramebuffer(gl.DRAW_FRAMEBUFFER,gpu.sceneTarget!.fbo);
                gl.blitFramebuffer(0,0,gpu.renderWidth,gpu.renderHeight,0,0,gpu.renderWidth,gpu.renderHeight,gl.COLOR_BUFFER_BIT,gl.NEAREST);
            }
            gl.bindFramebuffer(gl.READ_FRAMEBUFFER,gpu.sceneTarget!.fbo);gl.bindFramebuffer(gl.DRAW_FRAMEBUFFER,gpu.bloomA!.fbo);
            gl.blitFramebuffer(0,0,gpu.renderWidth,gpu.renderHeight,0,0,gpu.bloomA!.w,gpu.bloomA!.h,gl.COLOR_BUFFER_BIT,gl.LINEAR);
            gl.bindFramebuffer(gl.FRAMEBUFFER,(gpu.sceneMultisample??gpu.sceneTarget!).fbo);
            gl.depthMask(true);drawBuffer(gpu.instructionPieces,gl.TRIANGLES,programs.pieceProgram);gl.depthMask(false);
        }
        gl.disable(gl.DEPTH_TEST);gl.disable(gl.BLEND);
        if(gpu.sceneMultisample){
            gl.bindFramebuffer(gl.READ_FRAMEBUFFER,gpu.sceneMultisample.fbo);gl.bindFramebuffer(gl.DRAW_FRAMEBUFFER,gpu.sceneTarget!.fbo);
            gl.blitFramebuffer(0,0,gpu.renderWidth,gpu.renderHeight,0,0,gpu.renderWidth,gpu.renderHeight,gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT,gl.NEAREST);
        }
        ...
        gl.bindFramebuffer(gl.FRAMEBUFFER,gpu.sceneTarget!.fbo);gl.viewport(0,0,gpu.renderWidth,gpu.renderHeight);
        gl.enable(gl.DEPTH_TEST);gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,session.style.matte?gl.ONE_MINUS_SRC_ALPHA:gl.ONE);
        gl.useProgram(programs.typeProgram.p);gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,gpu.glyphTexture);gl.uniform1i(programs.typeProgram.u("uGlyphs"),0);
        drawBuffer(gpu.feedType,gl.TRIANGLES,programs.typeProgram);
        // 履歴から剥がれた文字は、ほどけている間はチップの手前に表示する。
        gl.disable(gl.DEPTH_TEST);drawBuffer(gpu.unravelType,gl.TRIANGLES,programs.typeProgram);
        gl.disable(gl.BLEND);
        gl.bindFramebuffer(gl.FRAMEBUFFER,null);gl.viewport(0,0,gpu.renderWidth,gpu.renderHeight);
        gl.useProgram(programs.compositeProgram.p);
        gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,gpu.sceneTarget!.texture);gl.uniform1i(programs.compositeProgram.u("uScene"),0);
        gl.activeTexture(gl.TEXTURE1);gl.bindTexture(gl.TEXTURE_2D,gpu.bloomB!.texture);gl.uniform1i(programs.compositeProgram.u("uBloom"),1);
        gl.uniform1i(programs.compositeProgram.u("uMatte"),session.style.matte?1:0);
        gl.uniform1f(programs.compositeProgram.u("uBloomAmount"),session.bloom);gl.uniform1f(programs.compositeProgram.u("uTime"),clock.artTime);
        gl.uniform1f(programs.compositeProgram.u("uShock"),session.reducedMotion?0:shock);gl.uniform2f(programs.compositeProgram.u("uResolution"),gpu.renderWidth,gpu.renderHeight);
        gl.uniform1f(programs.compositeProgram.u("uPixelRatio"),gpu.pixelRatio);
        gl.bindVertexArray(null);gl.drawArrays(gl.TRIANGLES,0,3);

    }
    return {render,buildMaterialShadow,
        get pieceShadows(){return {size:shadowPass.pieceShadow?.w??0,instances:session.style.matte?gpu.instructionPieces.count:0,updates:shadowPass.pieceShadowUpdates};}
    };
}
