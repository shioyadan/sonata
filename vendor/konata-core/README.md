...
`trace_parser.ts` の `onLine` callbackは、展開・行分割後の生記録をParserより先に同期通知します。Sonataの追加証拠抽出がFileとHTTPで同じ行を観測するための境界です。形式の再判定では先頭から再通知するため、集約状態を持つ呼出側は入力の`stream()`を開き直す時点で状態をリセットします。行ごとのPromiseや全文の複製は追加しません。
