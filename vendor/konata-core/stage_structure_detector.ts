import type { Op, ParsedTrace } from "./model";

export interface DetectedStage {
    readonly laneID: number;
    readonly stageName: string;
}

export interface DetectedStageGroup {
    readonly laneID: number;
    readonly stageNames: readonly string[];
}

export interface DetectedAllocationStage extends DetectedStageGroup {
    readonly width: number;
}

export interface DetectedAdmissionStage extends DetectedStage {
    readonly typicalLatency: number;
}

interface DetectedExecutionPath {
    readonly allocationStageName: string;
    readonly executionStageName: string;
    readonly completionStageNames: readonly string[];
}

export interface DetectedStageObservation {
    readonly allocationCycle: number | null;
    readonly issueCycle: number | null;
    readonly executionLatency: number | null;
    readonly completionCycle: number | null;
    readonly admissionStallStartCycle: number | null;
    readonly admissionStallEndCycle: number | null;
}

export class DetectedStageStructure {
    constructor(
        readonly allocationStage: Readonly<DetectedAllocationStage>,
        readonly executionStage: Readonly<DetectedStageGroup>,
        readonly transitionCoverage: number,
        readonly admissionStages: readonly Readonly<DetectedAdmissionStage>[],
        // 命令ごとの観測方法を非公開にし、Top-down分類へstage構造を漏らさない。
        private readonly executionPaths_: readonly Readonly<DetectedExecutionPath>[],
    ) {}

    observe(op: Readonly<Op>): DetectedStageObservation {
        const allocationNames = this.allocationStage.stageNames;
        let allocationCycle: number | null = null;
        let issueCycle: number | null = null;
        let executionLatency: number | null = null;
        let completionCycle: number | null = null;
        let admissionStallStartCycle: number | null = null;
        let admissionStallEndCycle: number | null = null;
        let selectedPath: Readonly<DetectedExecutionPath> | null = null;
        let awaitingCompletion = false;
        let previousRange: Readonly<StageRange> | null = null;

        visitStageRanges(op, this.allocationStage.laneID, (range) => {
            if (awaitingCompletion) {
                if (selectedPath?.completionStageNames.includes(range.name)) {
                    completionCycle = range.startCycle;
                }
                awaitingCompletion = false;
            }
            if (allocationCycle === null && allocationNames.includes(range.name)) {
                allocationCycle = range.startCycle;
                selectedPath = this.executionPaths_.find(
                    (path) => path.allocationStageName === range.name,
                ) ?? null;
                if (previousRange !== null) {
                    const admission = this.admissionStages.find(
                        (stage) => stage.laneID === this.allocationStage.laneID &&
                            stage.stageName === previousRange?.name,
                    );
                    if (admission !== undefined) {
                        admissionStallStartCycle = previousRange.startCycle +
                            admission.typicalLatency;
                        admissionStallEndCycle = Math.min(
                            previousRange.endCycle,
                            range.startCycle,
                        );
                    }
                }
            } else if (issueCycle === null && selectedPath !== null &&
                range.name === selectedPath.executionStageName) {
                issueCycle = range.startCycle;
                executionLatency = Math.max(0, range.endCycle - range.startCycle);
                awaitingCompletion = true;
            }
            previousRange = range;
        });
        return {
            allocationCycle,
            issueCycle,
            executionLatency,
            completionCycle,
            admissionStallStartCycle,
            admissionStallEndCycle,
        };
    }
}

interface StageState extends DetectedStage {
    readonly key: string;
    startCount: number;
    lastOp: number;
    // 正常リタイア命令について、開始順と終了順を逐次比較するための状態。
    lastStartCycle: number;
    maximumEndCycle: number;
    hasRetiredSample: boolean;
    invalid: boolean;
    exitInverted: boolean;
}

interface StageTransition {
    readonly from: StageState;
    readonly to: StageState;
    count: number;
    readonly latencies: Map<number, number>;
}

interface StageRange {
    readonly name: string;
    readonly startCycle: number;
    readonly endCycle: number;
}

interface StageStructureDraft {
    readonly allocationStage: Omit<DetectedAllocationStage, "width">;
    readonly executionStage: DetectedStageGroup;
    readonly transitionCoverage: number;
    readonly admissionStages: readonly DetectedAdmissionStage[];
    readonly executionPaths: readonly DetectedExecutionPath[];
}

function visitStageRanges(
    op: Readonly<Op>,
    laneID: number,
    visit: (range: Readonly<StageRange>) => void,
): void {
    const lane = op.lanes[laneID];
    if (lane === null || lane === undefined) {
        return;
    }
    let name = "";
    let startCycle = 0;
    let endCycle = 0;
    for (const stage of lane.stages) {
        const stageEnd = stage.endCycle === 0 ? op.retiredCycle : stage.endCycle;
        // 連続する同名stageは一回の滞留として扱う。
        if (stage.name === name) {
            startCycle = Math.min(startCycle, stage.startCycle);
            endCycle = Math.max(endCycle, stageEnd);
            continue;
        }
        if (name !== "") {
            visit({ name, startCycle, endCycle });
        }
        name = stage.name;
        startCycle = stage.startCycle;
        endCycle = stageEnd;
    }
    if (name !== "") {
        visit({ name, startCycle, endCycle });
    }
}

function getTypicalLatency(histogram: ReadonlyMap<number, number>): number {
    let typicalLatency = 0;
    let typicalCount = -1;
    for (const [latency, count] of histogram) {
        if (count > typicalCount || (count === typicalCount && latency < typicalLatency)) {
            typicalLatency = latency;
            typicalCount = count;
        }
    }
    return typicalLatency;
}

function mergeHistogram(target: Map<number, number>, source: ReadonlyMap<number, number>): void {
    for (const [value, count] of source) {
        target.set(value, (target.get(value) ?? 0) + count);
    }
}

/**
 * 候補stage集合が一つのallocation frontierになるかを再走査で検証する。
 * 候補ごとの最大幅を足さず、集合への実際の新規投入数から幅を求める。
 */
export class StageStructureMeasurement {
    private readonly allocationNames_: ReadonlySet<string>;
    private lastWidthCycle_ = Number.NEGATIVE_INFINITY;
    private startsInCycle_ = 0;
    private width_ = 0;
    private invalid_ = false;

    constructor(private readonly draft_: Readonly<StageStructureDraft>) {
        this.allocationNames_ = new Set(draft_.allocationStage.stageNames);
    }

    get allocationCandidate(): Readonly<DetectedStageGroup> {
        return this.draft_.allocationStage;
    }

    observe(op: Readonly<Op>): void {
        if (op.eof) {
            return;
        }
        let firstStartCycle: number | null = null;
        let allocationStageCount = 0;
        visitStageRanges(op, this.draft_.allocationStage.laneID, (range) => {
            if (!this.allocationNames_.has(range.name)) {
                return;
            }
            allocationStageCount++;
            firstStartCycle ??= range.startCycle;
        });

        // 正常経路で複数候補を直列に通る場合は、代替状態の集合とはみなさない。
        if (op.retired && !op.flush && allocationStageCount > 1) {
            this.invalid_ = true;
        }
        if (firstStartCycle === null) {
            return;
        }
        const widthCycle = Math.floor(firstStartCycle);
        if (!Number.isFinite(widthCycle) || widthCycle < this.lastWidthCycle_) {
            this.invalid_ = true;
        } else if (widthCycle === this.lastWidthCycle_) {
            this.width_ = Math.max(this.width_, ++this.startsInCycle_);
        } else {
            this.lastWidthCycle_ = widthCycle;
            this.startsInCycle_ = 1;
            this.width_ = Math.max(this.width_, 1);
        }
    }

    finish(): DetectedStageStructure | null {
        if (this.invalid_ || this.width_ <= 0) {
            return null;
        }
        return new DetectedStageStructure(
            {
                ...this.draft_.allocationStage,
                width: this.width_,
            },
            this.draft_.executionStage,
            this.draft_.transitionCoverage,
            this.draft_.admissionStages,
            this.draft_.executionPaths,
        );
    }
}

export type ProcessorFlowInstructionKind = "integer" | "branch" | "memory";
export type ProcessorFlowMachineOrder = "in-order" | "out-of-order";
export type ProcessorFlowNodeID =
    | `front-${number}`
    | "scheduler"
    | `exec-${ProcessorFlowInstructionKind}`
    | "memory-wait"
    | "completion"
    | "retire";

export interface DetectedProcessorFlowNode {
    readonly id: ProcessorFlowNodeID;
    readonly stageNames: readonly string[];
}

export interface DetectedProcessorFlowExecutionUnit extends DetectedProcessorFlowNode {
    readonly kind: ProcessorFlowInstructionKind;
    readonly pipeCount: number;
}

export interface DetectedProcessorFlowStage {
    readonly name: string;
    readonly node: ProcessorFlowNodeID;
    readonly startCycle: number;
    readonly endCycle: number;
}

export interface DetectedProcessorFlowObservation {
    readonly kind: ProcessorFlowInstructionKind;
    readonly stages: readonly Readonly<DetectedProcessorFlowStage>[];
    readonly allocationCycle: number | null;
    readonly issueCycle: number | null;
    readonly completionCycle: number | null;
    readonly memoryWaitStartCycle: number | null;
}

export interface ProcessorFlowStructureBuildOptions {
    readonly sampleOpCount?: number;
    readonly yieldInterval?: number;
    readonly isCanceled?: () => boolean;
}

interface ProcessorFlowTiming {
    readonly ranges: readonly Readonly<StageRange>[];
    readonly allocationIndex: number;
    readonly executionIndex: number;
    readonly allocationCycle: number | null;
    readonly issueCycle: number | null;
    readonly completionCycle: number | null;
}

interface RepresentativeStagePath {
    readonly laneID: number;
    readonly names: readonly string[];
    readonly count: number;
}

export function classifyProcessorFlowInstruction(labelName: string): ProcessorFlowInstructionKind {
    const separator = labelName.indexOf(":");
    const disassembly = (separator < 0 ? labelName : labelName.slice(separator + 1)).trim();
    const mnemonic = (disassembly.split(/\s+/, 1)[0] ?? "")
        .toLowerCase()
        .replace(/^c[._]/, "");
    if (/\bwrip\b/i.test(disassembly) ||
        /^(?:j|jr|jal|jalr|call|ret)$/.test(mnemonic) ||
        /^(?:b|bl|blr|br|bx|bal|beq|bne|beqz|bnez|blt|bge|bltu|bgeu|bgez|bltz|blez|bgtz|cbz|cbnz|tbz|tbnz)(?:\..*)?$/.test(mnemonic)) {
        return "branch";
    }
    if (/^(?:ld|ldr|ldp|ldur|ldxr|lw|lh|lb|lbu|lhu|sd|str|stp|stur|stxr|sw|sh|sb|load|store|prefetch|prfm)/.test(mnemonic)) {
        return "memory";
    }
    return "integer";
}

function getStageRanges(op: Readonly<Op>, laneID: number): readonly Readonly<StageRange>[] {
    const ranges: StageRange[] = [];
    visitStageRanges(op, laneID, (range) => ranges.push(range));
    return ranges;
}

function getProcessorFlowTiming(
    op: Readonly<Op>,
    laneID: number,
    allocationNames: ReadonlySet<string>,
    executionNames: ReadonlySet<string>,
    kind: ProcessorFlowInstructionKind,
    completionNames: ReadonlySet<string>,
): ProcessorFlowTiming {
    const ranges = getStageRanges(op, laneID);
    const allocationIndex = ranges.findIndex((range) => allocationNames.has(range.name));
    const executionIndex = allocationIndex < 0
        ? -1
        : ranges.findIndex((range, index) =>
            index > allocationIndex && executionNames.has(range.name));
    const allocation = ranges[allocationIndex];
    const execution = ranges[executionIndex];
    let completionIndex = executionIndex + 1;
    if (kind === "memory") {
        // memoryは共通execution直後でなく、Mcまたは最後のready stageまで実行中とみなす。
        const explicitMemoryCompletion = ranges.findIndex((range, index) =>
            index > executionIndex && range.name === "Mc");
        const orderedCompletionNames = [...completionNames];
        const preferredName = orderedCompletionNames[orderedCompletionNames.length - 1];
        completionIndex = explicitMemoryCompletion >= 0 ? explicitMemoryCompletion :
            ranges.findIndex((range, index) =>
                index > executionIndex && index < ranges.length - 1 && range.name === preferredName);
    }
    const completion = ranges[completionIndex];
    return {
        ranges,
        allocationIndex,
        executionIndex,
        allocationCycle: allocation?.startCycle ?? null,
        issueCycle: execution?.startCycle ?? null,
        completionCycle: completion?.startCycle ??
            (kind === "memory" || execution === undefined ? null : execution.endCycle),
    };
}

function getRepresentativeStagePath(
    ops: readonly Readonly<Op>[],
    laneID?: number,
    requiredNames?: ReadonlySet<string>,
): RepresentativeStagePath | null {
    const counts = new Map<string, RepresentativeStagePath>();
    for (const op of ops) {
        if (!op.retired || op.flush) {
            continue;
        }
        const firstLane = laneID ?? 0;
        const lastLane = laneID ?? op.lanes.length - 1;
        for (let currentLane = firstLane; currentLane <= lastLane; currentLane++) {
            const names = getStageRanges(op, currentLane).map((range) => range.name);
            if (names.length < 3 ||
                (requiredNames !== undefined &&
                    ![...requiredNames].every((name) => names.includes(name)))) {
                continue;
            }
            const key = `${currentLane}\u0000${names.join("\u0001")}`;
            const current = counts.get(key);
            if (current === undefined) {
                counts.set(key, { laneID: currentLane, names, count: 1 });
            }
            else {
                counts.set(key, { ...current, count: current.count + 1 });
            }
        }
    }
    let selected: RepresentativeStagePath | null = null;
    for (const candidate of counts.values()) {
        if (selected === null || candidate.count > selected.count ||
            (candidate.count === selected.count && candidate.names.length > selected.names.length)) {
            selected = candidate;
        }
    }
    return selected;
}

function getPeakOccupancy(
    intervals: readonly (readonly [number, number])[],
): number {
    const events = intervals.flatMap(([start, end]) =>
        end > start ? [[start, 1] as const, [end, -1] as const] : []);
    events.sort((left, right) => left[0] - right[0] || left[1] - right[1]);
    let current = 0;
    let peak = 0;
    for (const [, difference] of events) {
        current += difference;
        peak = Math.max(peak, current);
    }
    return peak;
}

function roundedVisibleCapacity(peak: number, minimum: number): number {
    return Math.min(64, Math.max(minimum, Math.ceil(peak / 8) * 8));
}

export class DetectedProcessorFlowStructure {
    private readonly allocationNames_: ReadonlySet<string>;
    private readonly executionNames_: ReadonlySet<string>;
    private readonly frontNodeByName_: ReadonlyMap<string, ProcessorFlowNodeID>;
    private readonly executionKinds_: ReadonlySet<ProcessorFlowInstructionKind>;
    private readonly completionNames_: ReadonlySet<string>;

    constructor(
        readonly machineOrder: ProcessorFlowMachineOrder,
        readonly laneID: number,
        readonly frontEnd: readonly Readonly<DetectedProcessorFlowNode>[],
        readonly allocationStage: Readonly<DetectedAllocationStage>,
        readonly executionUnits: readonly Readonly<DetectedProcessorFlowExecutionUnit>[],
        readonly completionStageNames: readonly string[],
        readonly retirementStageNames: readonly string[],
        readonly memoryBaseLatency: number | null,
        readonly schedulerCapacity: number,
        readonly completionCapacity: number,
    ) {
        this.allocationNames_ = new Set(allocationStage.stageNames);
        this.executionNames_ = new Set(executionUnits.flatMap((unit) => unit.stageNames));
        this.frontNodeByName_ = new Map(frontEnd.flatMap((node) =>
            node.stageNames.map((name) => [name, node.id] as const)));
        this.executionKinds_ = new Set(executionUnits.map((unit) => unit.kind));
        this.completionNames_ = new Set(completionStageNames);
    }

    observe(op: Readonly<Op>): DetectedProcessorFlowObservation {
        const kind = classifyProcessorFlowInstruction(op.labelName);
        const timing = getProcessorFlowTiming(
            op,
            this.laneID,
            this.allocationNames_,
            this.executionNames_,
            kind,
            this.completionNames_,
        );
        const executionKind = this.executionKinds_.has(kind)
            ? kind
            : this.executionUnits[0]?.kind ?? "integer";
        const memoryWaitStartCycle = kind === "memory" &&
            this.memoryBaseLatency !== null && timing.issueCycle !== null &&
            timing.completionCycle !== null &&
            timing.completionCycle - timing.issueCycle > this.memoryBaseLatency
            ? timing.issueCycle + this.memoryBaseLatency
            : null;
        const stages: DetectedProcessorFlowStage[] = [];
        const lastRangeIndex = timing.ranges.length - 1;
        for (let rangeIndex = 0; rangeIndex < timing.ranges.length; rangeIndex++) {
            const range = timing.ranges[rangeIndex];
            const boundaries = [range.startCycle, range.endCycle];
            if (memoryWaitStartCycle !== null &&
                memoryWaitStartCycle > range.startCycle && memoryWaitStartCycle < range.endCycle) {
                boundaries.push(memoryWaitStartCycle);
            }
            boundaries.sort((left, right) => left - right);
            for (let boundary = 0; boundary + 1 < boundaries.length; boundary++) {
                const startCycle = boundaries[boundary];
                const endCycle = boundaries[boundary + 1];
                let node: ProcessorFlowNodeID;
                if (timing.allocationIndex < 0 || rangeIndex < timing.allocationIndex) {
                    node = this.frontNodeByName_.get(range.name) ??
                        this.frontEnd[this.frontEnd.length - 1]?.id ?? "front-0";
                }
                else if (timing.issueCycle === null || startCycle < timing.issueCycle) {
                    node = "scheduler";
                }
                else if (rangeIndex === lastRangeIndex && op.retired && !op.flush &&
                    this.retirementStageNames.includes(range.name)) {
                    node = "retire";
                }
                else if (timing.completionCycle === null || startCycle < timing.completionCycle) {
                    node = memoryWaitStartCycle !== null && startCycle >= memoryWaitStartCycle
                        ? "memory-wait"
                        : `exec-${executionKind}`;
                }
                else {
                    node = "completion";
                }
                const previous = stages[stages.length - 1];
                if (previous?.node === node && previous.name === range.name) {
                    stages[stages.length - 1] = { ...previous, endCycle };
                }
                else {
                    stages.push({ name: range.name, node, startCycle, endCycle });
                }
            }
        }
        return {
            kind,
            stages,
            allocationCycle: timing.allocationCycle,
            issueCycle: timing.issueCycle,
            completionCycle: timing.issueCycle === null ? null : timing.completionCycle,
            memoryWaitStartCycle,
        };
    }
}

function yieldToBrowser(): Promise<void> {
    return new Promise<void>((resolve) => setTimeout(resolve, 0));
}

/** Trace先頭の有界sampleから、Flow表示に必要なstage構造だけを推定する。 */
export async function buildProcessorFlowStructure(
    trace: ParsedTrace,
    options: Readonly<ProcessorFlowStructureBuildOptions> = {},
): Promise<DetectedProcessorFlowStructure | null> {
    const sampleOpCount = Math.max(1, options.sampleOpCount ?? 50_000);
    const yieldInterval = Math.max(1, options.yieldInterval ?? 10_000);
    const detector = new StageStructureDetector();
    const ops: Readonly<Op>[] = [];
    // 非同期yield中にtraceが伸びても、この推定では開始時の範囲だけを走査する。
    const lastID = trace.lastID;
    let work = 0;
    for (let id = 0; id <= lastID && ops.length < sampleOpCount; id++) {
        if (options.isCanceled?.()) {
            return null;
        }
        const op = trace.getOpForScan(id);
        if (op !== undefined && !op.eof) {
            ops.push(op);
            detector.observe(op);
        }
        if (++work >= yieldInterval) {
            await yieldToBrowser();
            work = 0;
        }
    }
    if (ops.length === 0 || options.isCanceled?.()) {
        return null;
    }

    const measurement = detector.finish();
    if (measurement !== null) {
        for (const op of ops) {
            measurement.observe(op);
        }
    }
    const detected = measurement?.finish() ?? null;
    const candidate = detected?.allocationStage ?? measurement?.allocationCandidate;
    let representative = getRepresentativeStagePath(
        ops,
        candidate?.laneID,
        candidate === undefined ? undefined : new Set(candidate.stageNames),
    );
    representative ??= getRepresentativeStagePath(ops);
    if (representative === null) {
        return null;
    }

    const laneID = representative.laneID;
    let allocationNames = detected?.allocationStage.stageNames ?? candidate?.stageNames ?? [];
    const machineOrder: ProcessorFlowMachineOrder = detected === null ? "in-order" : "out-of-order";
    if (detected === null) {
        const durations = new Map<string, { total: number; count: number }>();
        const candidates = allocationNames.length === 0
            ? representative.names.slice(1, -2)
            : allocationNames;
        for (const op of ops) {
            for (const range of getStageRanges(op, laneID)) {
                if (!candidates.includes(range.name)) {
                    continue;
                }
                const current = durations.get(range.name) ?? { total: 0, count: 0 };
                current.total += Math.max(0, range.endCycle - range.startCycle);
                current.count++;
                durations.set(range.name, current);
            }
        }
        let selected = "";
        let selectedAverage = Number.NEGATIVE_INFINITY;
        for (const name of candidates) {
            const duration = durations.get(name);
            const average = duration === undefined ? 0 : duration.total / duration.count;
            if (average > selectedAverage) {
                selected = name;
                selectedAverage = average;
            }
        }
        if (selected === "") {
            return null;
        }
        allocationNames = [selected];
        representative = getRepresentativeStagePath(ops, laneID, new Set(allocationNames)) ?? representative;
    }

    const allocationSet = new Set(allocationNames);
    const allocationIndex = representative.names.findIndex((name) => allocationSet.has(name));
    if (allocationIndex <= 0 || allocationIndex + 1 >= representative.names.length) {
        return null;
    }
    const executionNames = detected?.executionStage.stageNames ??
        [representative.names[allocationIndex + 1]];
    const executionSet = new Set(executionNames);
    const executionIndex = representative.names.findIndex((name, index) =>
        index > allocationIndex && executionSet.has(name));
    if (executionIndex < 0) {
        return null;
    }

    const widthByCycle = new Map<number, number>();
    const issueStarts = new Map<ProcessorFlowInstructionKind, Map<number, number>>();
    const stageNamesByKind = new Map<ProcessorFlowInstructionKind, Set<string>>();
    const schedulerIntervals: Array<readonly [number, number]> = [];
    const completionIntervals: Array<readonly [number, number]> = [];
    const memoryLatencies: number[] = [];
    const completionStageNames = representative.names.slice(executionIndex + 1, -1);
    const completionSet = new Set(completionStageNames);
    for (const op of ops) {
        const kind = classifyProcessorFlowInstruction(op.labelName);
        const timing = getProcessorFlowTiming(
            op,
            laneID,
            allocationSet,
            executionSet,
            kind,
            completionSet,
        );
        if (timing.allocationCycle !== null) {
            const cycle = Math.floor(timing.allocationCycle);
            widthByCycle.set(cycle, (widthByCycle.get(cycle) ?? 0) + 1);
            schedulerIntervals.push([
                timing.allocationCycle,
                timing.issueCycle ?? op.retiredCycle,
            ]);
            completionIntervals.push([timing.allocationCycle, op.retiredCycle]);
        }
        if (timing.issueCycle === null) {
            continue;
        }
        let starts = issueStarts.get(kind);
        if (starts === undefined) {
            starts = new Map<number, number>();
            issueStarts.set(kind, starts);
        }
        starts.set(timing.issueCycle, (starts.get(timing.issueCycle) ?? 0) + 1);
        const executionName = timing.ranges[timing.executionIndex]?.name;
        if (executionName !== undefined) {
            const names = stageNamesByKind.get(kind) ?? new Set<string>();
            names.add(executionName);
            stageNamesByKind.set(kind, names);
        }
        if (kind === "memory" && timing.completionCycle !== null &&
            timing.completionCycle > timing.issueCycle) {
            memoryLatencies.push(timing.completionCycle - timing.issueCycle);
        }
    }

    const allocationWidth = detected?.allocationStage.width ??
        Math.max(1, ...widthByCycle.values());
    const executionUnits: DetectedProcessorFlowExecutionUnit[] = [];
    for (const kind of ["integer", "branch", "memory"] as const) {
        const starts = issueStarts.get(kind);
        if (starts === undefined) {
            continue;
        }
        executionUnits.push({
            id: `exec-${kind}`,
            kind,
            stageNames: [...(stageNamesByKind.get(kind) ?? executionSet)],
            pipeCount: Math.max(1, ...starts.values()),
        });
    }
    if (executionUnits.length === 0) {
        return null;
    }

    const frontEnd = representative.names.slice(0, allocationIndex)
        .map((name, index): DetectedProcessorFlowNode => ({
            id: `front-${index}`,
            stageNames: [name],
        }));
    return new DetectedProcessorFlowStructure(
        machineOrder,
        laneID,
        frontEnd,
        { laneID, stageNames: allocationNames, width: allocationWidth },
        executionUnits,
        completionStageNames,
        representative.names.slice(-1),
        memoryLatencies.length < 2 || Math.max(...memoryLatencies) === Math.min(...memoryLatencies)
            ? null
            : Math.min(...memoryLatencies),
        roundedVisibleCapacity(getPeakOccupancy(schedulerIntervals), 8),
        roundedVisibleCapacity(getPeakOccupancy(completionIntervals), 16),
    );
}

/**
 * Traceのstage名を仮定せず、allocation周辺のstage構造を推定する。
 *
 * Opをfile-local ID順に入力すると、各stageについて次のqueueらしい性質を調べる。
 *
 * - 命令がID順にstageへ入る（startCycleが逆転しない）
 * - 後の命令が先にstageを出ることがある（endCycleが一度でも逆転する）
 *
 * 同じlaneに複数候補がある場合は、二回目の走査で一命令が候補を一つだけ通ることと
 * 集合全体の投入順を確認し、ready／waitなどの代替状態を一つのfrontierにまとめる。
 * 前後遷移のない補助laneは候補にせず、命令列やcycle列は保持しない。
 */
export class StageStructureDetector {
    private readonly states_ = new Map<string, StageState>();
    private readonly transitions_ = new Map<string, StageTransition>();
    private observedOps_ = 0;

    observe(op: Readonly<Op>): void {
        if (op.eof) {
            return;
        }
        const observedOp = this.observedOps_++;
        // laneごとに、この命令が通過した各stageの滞留区間と直接遷移を観測する。
        for (let laneID = 0; laneID < op.lanes.length; laneID++) {
            let previousState: StageState | null = null;
            let previousRange: StageRange | null = null;
            visitStageRanges(op, laneID, (range) => {
                const state = this.observeStage_(
                    op,
                    observedOp,
                    laneID,
                    range.name,
                    range.startCycle,
                    range.endCycle,
                );
                if (previousState !== null && previousRange !== null) {
                    this.observeTransition_(
                        previousState,
                        state,
                        previousRange.startCycle,
                        previousRange.endCycle,
                        range.startCycle,
                    );
                }
                previousState = state;
                previousRange = range;
            });
        }
    }

    finish(): StageStructureMeasurement | null {
        const incoming = new Set<StageState>();
        const outgoing = new Set<StageState>();
        for (const transition of this.transitions_.values()) {
            outgoing.add(transition.from);
            incoming.add(transition.to);
        }
        const allocations = [...this.states_.values()].filter((state) =>
            !state.invalid && state.hasRetiredSample && state.exitInverted &&
            incoming.has(state) && outgoing.has(state),
        );
        if (allocations.length === 0) {
            return null;
        }
        const laneID = allocations[0].laneID;
        if (allocations.some((state) => state.laneID !== laneID)) {
            // laneをまたぐ候補は補助表示か独立queueかを区別できないため選ばない。
            return null;
        }

        const allocationKeys = new Set(allocations.map((state) => state.key));
        const executionPaths: DetectedExecutionPath[] = [];
        const executionNames: string[] = [];
        let selectedTransitionCount = 0;
        for (const allocation of allocations) {
            let execution: StageTransition | null = null;
            for (const transition of this.transitions_.values()) {
                if (transition.from !== allocation || allocationKeys.has(transition.to.key)) {
                    continue;
                }
                if (execution === null || transition.count > execution.count) {
                    execution = transition;
                }
            }
            if (execution === null) {
                return null;
            }
            selectedTransitionCount += execution.count;
            if (!executionNames.includes(execution.to.stageName)) {
                executionNames.push(execution.to.stageName);
            }
            const completionNames: string[] = [];
            for (const transition of this.transitions_.values()) {
                if (transition.from === execution.to &&
                    !completionNames.includes(transition.to.stageName)) {
                    completionNames.push(transition.to.stageName);
                }
            }
            executionPaths.push({
                allocationStageName: allocation.stageName,
                executionStageName: execution.to.stageName,
                completionStageNames: completionNames,
            });
        }

        // 複合frontierの入口latencyは、同じ前段から各代替状態への観測をまとめる。
        const admissionHistograms = new Map<string, {
            readonly stage: StageState;
            count: number;
            readonly latencies: Map<number, number>;
        }>();
        for (const transition of this.transitions_.values()) {
            if (!allocationKeys.has(transition.to.key) || allocationKeys.has(transition.from.key)) {
                continue;
            }
            let admission = admissionHistograms.get(transition.from.key);
            if (admission === undefined) {
                admission = {
                    stage: transition.from,
                    count: 0,
                    latencies: new Map<number, number>(),
                };
                admissionHistograms.set(transition.from.key, admission);
            }
            admission.count += transition.count;
            mergeHistogram(admission.latencies, transition.latencies);
        }
        const admissionStages = [...admissionHistograms.values()]
            .sort((left, right) => right.count - left.count)
            .map((admission) => ({
                laneID: admission.stage.laneID,
                stageName: admission.stage.stageName,
                typicalLatency: getTypicalLatency(admission.latencies),
            }));
        const startCount = allocations.reduce((sum, state) => sum + state.startCount, 0);
        return new StageStructureMeasurement({
            allocationStage: {
                laneID,
                stageNames: allocations.map((state) => state.stageName),
            },
            executionStage: {
                laneID,
                stageNames: executionNames,
            },
            transitionCoverage: Math.min(1, selectedTransitionCount / startCount),
            admissionStages,
            executionPaths,
        });
    }

    private getState_(laneID: number, stageName: string): StageState {
        const key = `${laneID}\u0000${stageName}`;
        let state = this.states_.get(key);
        if (state === undefined) {
            state = {
                key,
                laneID,
                stageName,
                startCount: 0,
                lastOp: -1,
                lastStartCycle: Number.NEGATIVE_INFINITY,
                maximumEndCycle: Number.NEGATIVE_INFINITY,
                hasRetiredSample: false,
                invalid: false,
                exitInverted: false,
            };
            this.states_.set(key, state);
        }
        return state;
    }

    private observeStage_(
        op: Readonly<Op>,
        observedOp: number,
        laneID: number,
        stageName: string,
        startCycle: number,
        endCycle: number,
    ): StageState {
        const state = this.getState_(laneID, stageName);
        if (state.lastOp === observedOp) {
            // 正常経路で同じstageが離れて再登場する構造は、単純なqueueとはみなさない。
            state.invalid ||= op.retired && !op.flush;
            return state;
        }
        state.lastOp = observedOp;
        state.startCount++;

        if (!op.retired || op.flush) {
            return state;
        }
        // 投入／退出順はsquash時刻に左右されない正常経路だけで判定する。
        if (!Number.isFinite(startCycle) || !Number.isFinite(endCycle) ||
            endCycle < startCycle || startCycle < state.lastStartCycle) {
            state.invalid = true;
        }
        if (state.hasRetiredSample && endCycle < state.maximumEndCycle) {
            // 後から入った命令が、それ以前の命令より先に出たことを示す。
            state.exitInverted = true;
        }
        state.lastStartCycle = Math.max(state.lastStartCycle, startCycle);
        state.maximumEndCycle = Math.max(state.maximumEndCycle, endCycle);
        state.hasRetiredSample = true;
        return state;
    }

    private observeTransition_(
        from: StageState,
        to: StageState,
        fromStartCycle: number,
        fromEndCycle: number,
        toStartCycle: number,
    ): void {
        const key = `${from.key}\u0001${to.key}`;
        let transition = this.transitions_.get(key);
        if (transition === undefined) {
            transition = { from, to, count: 0, latencies: new Map<number, number>() };
            this.transitions_.set(key, transition);
        }
        transition.count++;
        const latency = Math.min(fromEndCycle, toStartCycle) - fromStartCycle;
        if (Number.isFinite(latency) && latency >= 0) {
            transition.latencies.set(
                latency,
                (transition.latencies.get(latency) ?? 0) + 1,
            );
        }
    }
}
