...
// 再生時に結果が判明する時刻を追加する。
export function topDownObservationTimes(
    analysis: any,
    ops: Readonly<Op>[],
    first: number,
    last: number,
    ...
    const outcomes: number[][] = [],
        ...
        backend = new Set<number>();
    const previous = new Map<number, { start: number }>(),
        pending = new Map<number, { start: number; notice: number }>(),
        recoveries: Array<{ start: number; end: number; notice: number }> = [];
    const control = (label: string) => {
        const disassembly = label.slice(label.indexOf(":") + 1).trim(),
            mnemonic = (disassembly.split(/\s+/, 1)[0] ?? "").toLowerCase();
    ...
    for (const op of ops) {
        const o = analysis.structure.observe(op),
            cycle = o.allocationCycle === null ? null : Math.floor(o.allocationCycle);
        if (
            cycle !== null &&
            cycle >= first &&
            cycle <= last &&
        ...
        if (!op.flush && pending.has(op.tid)) {
            ...
            if (
                op.retired &&
                o.allocationCycle !== null &&
                o.allocationCycle >= recovery.start &&
                analysis.minimumRecoveryCycles !== null
            ) {
                recoveries.push({
                    ...recovery,
                    end: Math.min(o.allocationCycle, recovery.start + analysis.minimumRecoveryCycles)
                });
            }
        }
        ...
        if (
            ...
            op.retired &&
            o.allocationCycle !== null &&
            o.completionCycle !== null &&
            o.completionCycle > o.allocationCycle &&
            control(op.labelName)
        )
            previous.set(op.tid, { start: o.completionCycle });
    }
    // 解析器のスロット割り当てと同じ命令の走査順・幅の上限を使う。
    for (let i = 0; i < slots.length; i++)
        for (const category of [0, 1]) {
            if (outcomes.filter((e) => e[0] === first + i && e[2] === category).length !== slots[i][category])
                throw new Error(`Top-down outcome evidence disagrees at ${first + i}, class ${category}`);
        }
    const recoveryNotices = slots.map((row, i) => {
        if (!row[2]) return null;
        const cycle = first + i,
            matching = recoveries.filter((r) => cycle >= Math.floor(r.start) && cycle < Math.ceil(r.end));
