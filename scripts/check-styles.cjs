...
module.exports=async function reviewStyles(window,entry,screenshots){
    ...
    const capture=async name=>{
        await settle();
        fs.writeFileSync(path.join(screenshots,`sonata-style-${name}.png`),(await window.webContents.capturePage()).toPNG());
        ...
        const pixels=require('electron').nativeImage.createFromDataURL(png).toBitmap(),background=[pixels[0],pixels[1],pixels[2]];
        let different=0,total=0;
        for(let i=0;i<pixels.length;i+=4*13){
            if(Math.hypot(pixels[i]-background[0],pixels[i+1]-background[1],pixels[i+2]-background[2])>30)different++;
            total++;
        }
        assert.ok(different/total>.015,`${name}: canvas is nearly blank`);
    ...
    assert.equal(await js("document.querySelector('#marble-look,#look-toggle')"),null,"Prototype instruction controls remain visible");
    assert.ok(await js(`(()=>{
    ...
    const scenes=[];let fixedRadius;
    for(const key of await js("embeddedFlowTraces.map(t=>t.key)")){
        const checkpoints=await js(`sonata.loadTrace(${JSON.stringify(key)});[sonata.trace.demo.screenshotCycle,...sonata.trace.demo.bookmarks.map(b=>b.cycle+.4)]`);
        ...
        const layout=await js("sonata.instructionLayout"),spacing={};
        for(const name of ['scheduler','rob','rename']){
            let minimum=Infinity;const positions=layout[name];
            for(let i=0;i<positions.length;i++)for(let j=i+1;j<positions.length;j++)
                minimum=Math.min(minimum,Math.hypot(positions[i][0]-positions[j][0],positions[i][2]-positions[j][2]));
            assert.ok(minimum>layout.radius*2,`${key}: ${name} slots overlap fixed-size instructions`);spacing[name]=minimum;
        }
        for(const cycle of checkpoints){
            ...
            assert.deepEqual(result.radii,[fixedRadius],`${key} @ ${cycle}: instruction size changed with stage or state`);
            ...
            assert.ok(result.sameTrace&&result.pieces>0);assert.equal(result.error,0);
            assert.equal(result.shape,"cut-crystal",`${key} @ ${cycle}: instruction shape changed`);
        }
        await js(`sonata.captureAt(sonata.trace.demo.screenshotCycle);document.querySelector('.telemetry').scrollTop=0`);
        const neon=await capture(`${key}-neon`);
        await js("reviewStyle('blocks')");const blocks=await capture(`${key}-blocks`);
        assert.ok(blocks.background.reduce((s,v)=>s+v,0)>neon.background.reduce((s,v)=>s+v,0)+300,"Style did not change the rendered canvas");
    ...
    assert.ok(Math.hypot(...rolling.moving[0].position.map((v,i)=>v-rolling.moving[1].position[i]))>.1,"Moving fixture did not move");
    assert.ok(Math.hypot(...rolling.moving[0].rotation.map((v,i)=>v-rolling.moving[1].rotation[i]))>.1,"Moving instruction did not roll");
    assert.ok(rolling.rewound&&rolling.stylePreserved&&rolling.reloaded&&rolling.normalized&&rolling.squashed>0,"Rolling lost deterministic, finite poses");
    const pausedPieces=await js("sonata.pieces");await settle();
    ...
    await waitFor(`sonata.cycle>${running.after.cycle+.2}`,"Playback did not continue after switching");
    await js("document.getElementById('bloom').value='123';document.getElementById('bloom').dispatchEvent(new Event('input'))");
    await js("sonata.setPlaying(false);reviewStyle('blocks')");
    assert.equal(await js("document.getElementById('bloom').disabled"),true,"Blocks left an ineffective bloom slider enabled");
    await js("reviewStyle('neon')");
    assert.equal(await js("!document.getElementById('bloom').disabled&&document.getElementById('bloom').value==='123'&&document.getElementById('bloom-value').textContent==='123%'"),true,"Neon lost the user's bloom setting");
    ...
    return {scenes,fixedRadius,pieceShadows,grounding,rolling:{...rolling,paused:true,pixels:posePixels},selectionPreserved:true,playingPreserved:true,mobile};
};
