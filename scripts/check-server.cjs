...
async function main(){
    const child=spawn(process.execPath,[path.join(root,"scripts/serve.cjs")],{
        cwd:root,env:{...process.env,SONATA_HOST:"127.0.0.1",SONATA_PORT:"0"},stdio:["ignore","pipe","pipe"]
    });
    let output="",errors="";
    ...
    try{
        const port=await new Promise((resolve,reject)=>{
            ...
            child.once("exit",code=>{clearTimeout(timer);reject(new Error(`Preview server exited: ${code}; ${errors}`));});
        });
        const request=(target,method="GET")=>new Promise((resolve,reject)=>{
            const req=http.request({hostname:"127.0.0.1",port,path:target,method,timeout:5000},res=>{
                const chunks=[];
        ...
        for(const target of ["/","/sonata.html","/?demo=1"]){
            ...
            assert.equal(response.status,200);assert.deepEqual(response.body,html);
