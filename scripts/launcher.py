...
REQUIRED_FILES = {
    "sonata.sh", "scripts/launcher.py", "sonata.html", "README.md",
...
def unpack_update(archive_path, payload):
    with zipfile.ZipFile(archive_path) as archive:
        ...
        for entry in entries:
            ...
            names.add(name)
            if entry.is_dir():
                if name not in ("sonata-latest/", "sonata-latest/scripts/", "sonata-latest/samples/"):
    ...
    if not script.read_text(encoding="utf-8").startswith("#!/usr/bin/env bash\n"):
        raise ValueError("Invalid sonata.sh")
    subprocess.run(["bash", "-n", str(script)], check=True, capture_output=True)
    ast.parse((payload / "scripts/launcher.py").read_text(encoding="utf-8"))
    return manifest
