...
app.whenReady()
    .then(async () => {
        ...
        await settle();
        fs.writeFileSync(
            path.join(screenshots, "sonata-licenses.png"),
        ...
        for (const key of keys) {
            ...
            assert.equal(
                ...
                true,
                `${key}: commit slots must match the actual in-order commit group`
            );
            assert.equal(
                result.feedMatches,
                true,
                `${key}: incoming stream must use actual upcoming trace instructions`
            );
            ...
            await settle();
            fs.writeFileSync(
                path.join(screenshots, `sonata-${key}.png`),
                (await window.webContents.capturePage()).toPNG()
            );
        }
        ...
        assert.deepEqual(
            await js("sonata.instructionFeed"),
            feedBefore,
            "Instruction stream did not return when enabled"
        );
        await settle();
        ...
        assert.equal(
            await js("document.getElementById('op-id').textContent"),
            `#${pick.id}`,
            "Particle picking failed"
        );
        ...
        for (const event of unravel) {
            ...
            assert.equal(
                event.correct,
                true,
                "Detached glyphs must come from this flush's actual canceled instructions"
            );
            assert.ok(
        ...
        for (const r of renameWordReview) {
            assert.ok(
                r.first.progress < r.second.progress,
                "Rename word update should sweep smoothly through the cells"
            );
        ...
        assert.deepEqual(
            issueReview.signal[0],
            issueReview.exit,
            "Issue column signal must start at the right-hand exit"
        );
        ...
        fs.writeFileSync(
            path.join(screenshots, "sonata-top-down.png"),
            (await window.webContents.capturePage()).toPNG()
        );
        for (const state of bounds.states.filter((s) =>
            ...
            if (state.dominant === "frontend")
                assert.ok(layout.bounds[1] < 0, "Frontend emphasis spread into the backend");
            if (state.dominant === "backend")
                assert.ok(layout.bounds[0] > -6, "Backend emphasis spread into the frontend");
            fs.writeFileSync(
            ...
            JSON.stringify(
                {
                    ...
                    registerReadReview,
                    recovery: {
                        frames: recoveryFrames.length,
                        rates: recoveryFrames.map((f) => ({ age: f.age, rate: f.rate }))
                    },
                    notification,
                    rsd,
                    bounds,
                    boundMotion: {
                        frames: boundMotion.samples.length,
                        intermediate: intermediate.length,
                        reducedMotionExact: true
                    },
                    motion,
