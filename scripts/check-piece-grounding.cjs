...
near(a.position[1], 0.4 + 0.88 * 0.2, "Crystal is not seated");
...
near(ground.seat([0, 0, 0], 0.2, quarter).position[1], 0.4 + 0.88 * 0.2, "Quarter-turn changed crystal height");
near(
    ground.seat([0, 0, 0], 0.2, eighth).position[1],
    0.4 + (0.2 * 1.2) / Math.SQRT2,
    "Rotating crystal clips its facets"
);
...
near(slope.seat([0, 10, 0], 0.2, identity).position[1], 0.2 * (0.88 + 0.5 * 0.32), "Crystal penetrates sloped trough");
near(slope.seat([0, 10, 0], 0.2, identity, true).position[1], 0.2 * Math.sqrt(1.25), "Sphere penetrates sloped trough");
// 台の外へ出ると縁を支点に下がり、離れた後は下の面へ接する。
const edge = createGround(plane(0.4, -3, 0), 0);
near(
    edge.seat([0.12, 8, 0], 0.2, identity, true).position[1],
    0.4 + Math.sqrt(0.2 ** 2 - 0.12 ** 2),
    "Sphere floats at a ledge",
    2e-6
);
near(edge.seat([0.21, 8, 0], 0.2, identity, true).position[1], 0.2, "Sphere did not reach the lower surface");
near(
    edge.seat([0.12, 8, 0], 0.2, identity).position[1],
    0.4 + 0.2 * (1.2 - 0.12 / 0.2),
    "Crystal floats at a ledge",
    2e-6
);
near(edge.seat([0.18, 8, 0], 0.2, identity).position[1], 0.88 * 0.2, "Crystal did not reach the lower surface");
...
console.log(
    "Piece grounding: cut crystal and clearance sphere; rotation; sloped surfaces; ledges; deterministic contact"
);
