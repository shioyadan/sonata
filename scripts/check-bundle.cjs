...
try {
    ...
    const source = bundle(root, "main.js"),
        result = vm.runInNewContext(source);
    ...
    assert.deepEqual(
        ...
        "TypeScript entry must execute the same module graph"
    );
    assert.ok(!source.includes('"unused.d.cts":'), "Type declarations must not become runtime modules");
    for (const [specifier, error] of [
        ["./missing.js", /Unknown bundled module/],
        ["node:fs", /Expected a relative module/],
        ["../outside.js", /outside the source directory/]
    ]) {
    ...
    console.log(
        "Bundled modules: mixed JS/TypeScript, relative paths, scope, cache, cycles, isolation and invalid imports passed"
    );
} finally {
