...
module.exports = async function reviewBrowser(window, entry, screenshots) {
    ...
    const ready = () =>
        waitFor(
            "!!globalThis.sonata&&sonata.renderer.error===0&&document.getElementById('fallback').hidden",
    ...
    const press = async (keyCode, modifiers = []) => {
        ...
        await js(
            "globalThis.reviewKeyReleased=false;document.addEventListener('keyup',()=>globalThis.reviewKeyReleased=true,{once:true,capture:true})"
        );
    ...
    const first = await js(
        "sonata.setPlaying(false);sonata.setCycle(sonata.trace.firstCycle);document.getElementById('scene').focus();sonata.cycle"
    );
    await press("Right");
    assert.equal(await js("sonata.cycle"), first + 1, "Right arrow did not step one cycle");
    await press("Left");
    assert.equal(await js("sonata.cycle"), first, "Left arrow did not step back");
    await press("Left");
    assert.equal(await js("sonata.cycle"), first, "Left arrow escaped the trace start");
    await js("sonata.setCycle(sonata.trace.lastCycle)");
    await press("Right");
    assert.equal(await js("sonata.cycle===sonata.trace.lastCycle"), true, "Right arrow escaped the trace end");
    await js("sonata.setCycle(sonata.trace.firstCycle)");
    await press("Space");
    assert.equal(await js("sonata.playing"), true, "Space did not start playback");
    await press("Space");
    assert.equal(await js("sonata.playing"), false, "Space did not pause playback");
    await press("C");
    assert.equal(await js("sonata.camera.mode"), "cinema", "C did not open Cinema");
    await press("Escape");
    assert.equal(await js("sonata.camera.mode"), "orbit", "Escape did not leave Cinema");

    // 入力欄の矢印操作は、その値だけを変える。全体の1サイクル移動と混同しない。
    const cycle = await js(
        "sonata.setCycle(sonata.trace.firstCycle+10);document.getElementById('bloom').value='100';document.getElementById('bloom').focus();sonata.cycle"
    );
    await press("Right");
    assert.equal(
        await js("document.getElementById('bloom').value"),
        "101",
        "Focused range did not receive the arrow key"
    );
    assert.equal(await js("sonata.cycle"), cycle, "A focused range also stepped playback");
    await js("document.getElementById('timeline').focus()");
    await press("Right");
    assert.ok(
        Math.abs((await js("sonata.cycle")) - cycle - 0.01) < 1e-7,
        "Timeline key also triggered the global cycle shortcut"
    );
    await js("document.getElementById('speed').focus()");
    await press("C");
    assert.equal(await js("sonata.camera.mode"), "orbit", "A focused select activated Cinema");

    // button の Space はブラウザ標準の click を一度だけ発生させる。
    await js("sonata.setPlaying(false);document.getElementById('play').focus()");
    await press("Space");
    assert.equal(await js("sonata.playing"), true, "Focused play button toggled playback twice");
    await press("Space");
    assert.equal(await js("sonata.playing"), false);
    await js("document.getElementById('license-open').focus()");
    await press("Space");
    assert.equal(
        await js(
            "document.getElementById('license-panel').open&&document.getElementById('license-panel').contains(document.activeElement)"
        ),
        true,
    ...
    await press("Tab");
    assert.equal(
        await js("document.getElementById('license-panel').contains(document.activeElement)"),
        true,
        "Tab escaped the modal"
    );
    await press("C");
    assert.equal(await js("sonata.camera.mode"), "orbit", "Modal key activated the global shortcut");
    await press("Escape");
    assert.equal(
        await js("!document.getElementById('license-panel').open&&document.activeElement.id==='license-open'"),
        true,
    ...
    await js(
        "sonata.captureAt(sonata.trace.demo.screenshotCycle);if(document.getElementById('auto-camera').getAttribute('aria-pressed')==='true')document.getElementById('auto-camera').click();document.getElementById('zoom-fit').click()"
    );
    await waitFor(
        "Math.abs(sonata.camera.radius-32.5)<.01&&Math.abs(sonata.camera.azimuth-.2)<.001",
        "Fit did not settle before zoom input"
    );
    const cameraBefore = await js("sonata.camera");
    const area = await js(
        "(()=>{const r=document.getElementById('scene').getBoundingClientRect();return {x:Math.round(r.left+r.width*.5),top:Math.round(r.top+r.height*.15),bottom:Math.round(r.top+r.height*.85)};})()"
    );
    const drag = async (from, to, button = "right") => {
        await js(
            "globalThis.reviewPointerReleased=false;document.getElementById('scene').addEventListener('pointerup',()=>globalThis.reviewPointerReleased=true,{once:true})"
        );
        ...
        await waitFor("globalThis.reviewPointerReleased", "Zoom drag did not release its pointer");
        await settle();
    ...
    await js(
        "document.getElementById('zoom-fit').click();for(let i=0;i<20;i++)document.getElementById('zoom-in').click()"
    );
    ...
    await js(
        "document.getElementById('zoom-fit').click();globalThis.reviewWheelReceived=false;document.getElementById('scene').addEventListener('wheel',()=>globalThis.reviewWheelReceived=true,{once:true})"
    );
    window.webContents.sendInputEvent({
        type: "mouseWheel",
        x: area.x,
    ...
    try {
        ...
        await window.loadFile(entry);
        assert.equal(
            await js("document.getElementById('fallback').hidden"),
            false,
            "Missing WebGL did not show the fallback"
        );
        ...
        await press("Escape");
        assert.equal(await js("document.getElementById('license-panel').open"), false);
    } finally {
        try {
            if (injected)
    ...
    for (const visualStyle of ["neon", "blocks"]) {
        ...
        await waitFor(
            "document.getElementById('renderer-status').textContent==='Graphics context lost'",
            "Context loss was not reported"
        );
        assert.equal(await js("document.getElementById('fallback').hidden"), false);
        const stopped = await js("sonata.cycle");
        await settle();
        ...
        await waitFor(
            "!!globalThis.sonata&&document.getElementById('fallback').hidden&&!document.getElementById('scene').getContext('webgl2').isContextLost()",
    ...
    try {
        ...
        await waitFor(
            "Math.abs(sonata.camera.radius-sonata.camera.targetRadius)<.01",
            "Crystal close-up without MSAA did not settle"
        );
        withoutMSAA = {
            ...(await readFrame()),
            ...(await js("({samples:sonata.renderer.msaaSamples,pieces:sonata.renderer.pieceInstances})"))
        };
        ...
        assert.equal(
            await js("sonata.renderer.instructionShape"),
            "cut-crystal",
