...
const {createRollingTrack}=require("../src/piece-motion.js");
...
const back=createRollingTrack(t=>[quarter*(t<1?t:t<3?1:4-t),0,0],[0,1,3,4],{distancePerRadian:1});
...
const corner=createRollingTrack(t=>[quarter*Math.min(t,1),0,quarter*Math.max(t-1,0)],[0,1,2],{distancePerRadian:1});
...
const path=t=>[Math.cos(t*2),t*.2,Math.sin(t*2)];
const curve=createRollingTrack(path,[0,.5,1,1.5,2]);
const direct=curve.rotationAt(.731);
for(let t=0;t<2;t+=1/120)curve.rotationAt(t);
for(let t=2;t>0;t-=1/15)curve.rotationAt(t);
assert.deepEqual(curve.rotationAt(.731),direct,"Rolling depends on seek direction or frame rate");
const fresh=createRollingTrack(path,[0,.5,1,1.5,2]);
assert.deepEqual(fresh.rotationAt(.731),direct,"A freshly loaded path has a different orientation");
for(let t=0;t<=2;t+=.031)assert.ok(Math.abs(Math.hypot(...curve.rotationAt(t))-1)<1e-12,"Rotation lost normalization");
const modified=curve.rotationAt(.5);modified.fill(0);
assert.ok(Math.hypot(...curve.rotationAt(.5))>.999,"Reading orientation exposed mutable cached data");
console.log("Piece rolling: direction; distance; corners; waiting; reversal; deterministic seeks; normalized rotations");
