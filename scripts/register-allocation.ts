...
export function allocationEvidence(
    ...
    firstCycle: number,
    lastCycle: number,
    kind: "recorded" | "inferred",
    label: string
) {
    const initial = new Map<number, string>();
    for (const event of events) {
        if (event.cycle >= firstCycle) break;
        initial.set(event.physical, event.state);
    }
    return {
        kind,
        label,
        initial: [...initial],
        events: events.filter((e) => e.cycle >= firstCycle && e.cycle <= lastCycle)
    };
}
