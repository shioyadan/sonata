    ...
    function run(modules,entry){
        ...
        function load(file){
            ...
            const require=specifier=>{
                if(!specifier.startsWith("./")&&!specifier.startsWith("../"))throw new Error(`Expected a relative module: ${specifier}`);
                const parts=file.split("/");parts.pop();
                for(const part of specifier.split("/")){
                    if(part===".."){if(!parts.length)throw new Error("Module is outside the source directory");parts.pop();}
