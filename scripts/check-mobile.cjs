    ...
    try {
        ...
        for (const [width, height, name] of [
            ...
            for (const key of styleSequence) {
                ...
                assert.ok(
                    await js(
                        `document.querySelectorAll('[data-style-choice][aria-pressed=true]').length===1&&document.getElementById('style-'+${JSON.stringify(key)}).getAttribute('aria-pressed')==='true'`
                    ),
                    "Style buttons lost their selection state"
                );
            }
        }
        ...
        await delay(600);
        const center = await js(
            `(()=>{const r=document.getElementById('scene').getBoundingClientRect();return {x:r.left+r.width/2,y:r.top+r.height/2};})()`
        );
        const point = (id, x, y) => ({ id, x, y, radiusX: 5, radiusY: 5, force: 1 });
        const pair = (half, dx = 0, dy = 0) => [
            point(1, center.x - half + dx, center.y + dy),
            point(2, center.x + half + dx, center.y + dy)
        ];
        ...
        for (let i = 1; i <= 5; i++) {
            await touch("touchMove", pair(40 + i * 8));
            await settle();
        }
        const pinched = await js("sonata.camera");
        assert.ok(pinched.targetRadius < before.targetRadius * 0.7, "Pinch did not enlarge the pipeline");
        ...
        for (let i = 1; i <= 4; i++) {
            await touch("touchMove", pair(80, i * 8, 0));
            await settle();
        }
        const panned = await js("sonata.camera");
        assert.ok(
            Math.hypot(...panned.targetFocus.map((v, i) => v - pinched.targetFocus[i])) > 0.5,
            "Two-finger movement did not pan"
        );
        assert.ok(Math.abs(panned.targetRadius - pinched.targetRadius) < 0.001, "A parallel pan changed zoom");
        ...
        await delay(600);
        assert.ok(
            Math.abs((await js("sonata.camera.azimuth")) - angle) > 0.03,
            "One-finger orbit did not work after pinch"
        );
        ...
        for (let i = 1; i <= 6; i++) {
            await touch("touchMove", pair(8 + i * 20));
            await settle();
        }
        ...
        assert.ok(
            await js(
                "document.getElementById('mobile-panel').open&&document.getElementById('mobile-panel').contains(document.activeElement)"
            )
        );
        await js(
            "document.getElementById('license-open').scrollIntoView();document.getElementById('license-open').click()"
        );
        assert.ok(
            await js(
                `(()=>{const panel=document.getElementById('license-panel'),r=panel.getBoundingClientRect();return panel.open&&r.left>=0&&r.right<=innerWidth&&r.top>=0&&r.bottom<=innerHeight&&panel.scrollWidth<=panel.clientWidth;})()`
            ),
            "License notices overflow the phone viewport"
        );
        await settle();
        await capture("-licenses");
        await js(
            "document.querySelector('#license-panel button').click();document.getElementById('mobile-panel').scrollTop=0"
        );
        assert.ok(
        ...
        assert.ok(
            await js(
                "document.getElementById('mobile-panel').scrollWidth<=document.getElementById('mobile-panel').clientWidth"
            ),
            "Run details overflow the sheet"
        );
        ...
        await settle();
        await capture("-flush");
        await js("document.querySelector('[data-view=cinema]').click()");
        await delay(500);
        assert.ok(
            await js("document.getElementById('play').getBoundingClientRect().bottom<=innerHeight"),
            "Cinema lost playback controls"
        );
        await js("document.getElementById('mobile-details').click();document.getElementById('show-highlight').click()");
        assert.ok(
            await js("!document.getElementById('mobile-panel').open&&sonata.playing"),
            "Highlight stayed hidden behind the sheet"
        );
        ...
        result = {
            layouts,
            touch: { pinch: true, detailZoom: true, pan: true, orbit: true, cancel: true, fit: true },
            demos: keys.length
        };
    } finally {
        try {
