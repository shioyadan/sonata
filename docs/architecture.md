...
`exhibition.cts` は展示の状態と巡回順序・無操作期限を所有し、サンプル読込みや表示切替は必要なコールバックで依頼します。DOMの見た目、Fullscreen、入力の捕捉、再生時計と30fpsの描画上限は `sonata.cts` がつなぎます。展示専用の再生モデルやサンプル解析器は作らず、通常の `demo-loader` と同じ取得・取消し・キャッシュを使います。キャッシュ上限は一覧の6デモに合わせ、最初の一巡後に再解析し続けないようにします。全ファイルの事前取得は行いません。
