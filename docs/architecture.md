...
一つの変更を少数のファイルで理解・完結できることを優先します。同じ状態を使い、一緒に変更する処理はまとめます。10ファイルは現在の構成を表し、将来の固定上限ではありません。新しい処理の責務と変更のまとまりに応じて、既存ファイルへの追加と分割を判断します。ファイル数を減らすために実行時のソースを `scripts/` へ移すことはしません。

型は担当する処理と同じファイルに置き、公開する型だけをモジュールの型用 namespace にまとめます。GPU のファイルには GLSL も含みます。ファイル内の名前付き関数と短い区切りのコメントで役割を追えるようにし、小さな行数上限を分割理由にしません。独立して変わる責務が増えた場合は、分割の理由を説明して構成を見直します。汎用レジストリ、イベントバス、継承階層は現状では不要です。
