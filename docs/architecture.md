...
編集用の `src/` は JavaScript 6個、CSS 3個、HTML 1個の計10ファイルです。配布時には `dist/sonata.html` 一つへ結合します。画面の配置・記録値・再生時刻と、外観の材質・照明・合成を分けることで、Neon / Blocks を同じ再生内容で比較できます。
...
| `src/replay-model.js` | デモの準備、記録時刻に対応する FIFO・依存・レジスタ・Top-down の状態 |
| `src/geometry.js` | 座標・行列、命令の経路とステージ補間、回転・接地と姿勢キャッシュ |
...
JavaScript は約370〜640行です。GPU のファイルには GLSL も含みます。ファイル内の名前付き関数と短い区切りのコメントで役割を追えるようにし、小さな行数上限を分割理由にしません。逆に、独立して変わる責務が増えた場合は10個を絶対条件にせず、分割の理由を説明して構成を見直します。汎用レジストリ、イベントバス、継承階層は現状では不要です。
...
ブラウザ用 JavaScript は、拡張子を含む相対パスの CommonJS で参照します。`scripts/bundle.cjs` は `src/` 内の JavaScript を関数単位のモジュールとして包み、`sonata.js` から実行するスクリプトを生成します。各モジュールは必要になったときに一度だけ実行されます。文字列の単純連結による変数共有や `eval` は使いません。
