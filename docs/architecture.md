...
`replay.schedulers` は表示する待機列のID・全体スロット番号の開始位置・表示行数を持ちます。命令の `issueSlot` と依存列番号は全体で一意にし、sceneが待機列内の座標へ変換します。`issue-fp` はFP／SIMDの待機段、`exec-fp` はその実行段です。実機キュー容量やFPレジスタの観測とは区別します。
