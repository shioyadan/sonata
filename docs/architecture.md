...
| `src/demo-loader.cts` | 固定一覧、サンプル解析Workerの寿命・取消し、導出済みTrace最大6件のキャッシュ |
| `src/exhibition.cts` | 展示の巡回、探索への遷移、無操作時の復帰、読込み失敗後の再試行 |
...
| `src/frontend.cts` | Fetchサイクル単位の束、前段で共有するlane、表示行の時刻索引と前詰め |
...
`exhibition.cts` は展示の状態と巡回順序・無操作期限を所有し、サンプル読込みや表示切替は必要なコールバックで依頼します。表示の開始・探索からの復帰・次デモへの移行をコールバックの理由で区別し、スタイルの乱択と手動設定の復元は `sonata.cts` が担当します。DOMの見た目、Fullscreen、入力の捕捉、再生時計と30fpsの描画上限は `sonata.cts` がつなぎます。展示専用の再生モデルやサンプル解析器は作らず、通常の `demo-loader` と同じ取得・取消し・キャッシュを使います。キャッシュ上限は一覧の6デモに合わせ、最初の一巡後に再解析し続けないようにします。全ファイルの事前取得は行いません。
...
`replay.frontend` は `frontend.cts` で準備します。各段の滞在と退出補間から束の予約区間を作り、古い束の予約数を時刻索引で引いて表示行を求めます。描画ごとに全命令を並べ直さず、任意時刻へのシークでも同じ位置を返します。sceneは束数と共通lane数から台の寸法を決め、geometryは `position(id, node, time)` の行・laneを座標へ変換します。空きスロットの再利用や命令indexの剰余で前段を配置しません。

Fだけは4束の固定表示枠を使います。frontendが表示入場時刻を事前計算し、次段への記録上の移動時刻で枠を解放します。geometryは入場前の駒を描かず、入場からF終了までの時間に合わせて移動を補間します。同サイクルの入退場は次段の入場補間内でFを経由させ、束ごとに通過位置と時間を分けます。activityは入場待ちのIDを既存の文字帯の先頭へ置き、残る行だけを先読みに使います。文字帯の行数は増やさず、元のstage・占有・再生時計は変更しません。
...
FileとHTTPの生トレースは、同じWorker・Konata core・`trace-window.cts`を通します。デモだけの解析済みJSON経路はありません。サンプル解析後は表示区間から導出したTraceだけを最大6件キャッシュし、解析Workerと全体storeを解放します。Fileの全体は操作を続けるためWorkerのKonata storeへ保持し、選択区間だけを描画へ渡します。未観測値、圧縮、生成済みCoreの扱いは[ローカルトレースの読込み](trace-import.md)を参照してください。
